...
## 2. Configuration Files Have Changed

In v2, almost everything was jammed into a single configuration file, with a separate file for messages (`CommandPrompter-Messages_en.yml`). 

In v3, the configuration is neatly split into two files:

*   **`config.yml`**: Contains your core plugin behavior (`Prompt-Prefix`, `Prompt-Timeout`, `Cancel-Keyword`), ignored commands, delegation settings, and **all your messages**. The old `CommandPrompter-Messages_en.yml` is gone; its contents live here now.
*   **`prompt-config.yml`**: Contains all your UI/Screen settings (Anvils, Signs, Player pickers, and the new Dialogs). You generally edit this file less often, which is why it was separated.

**Migration Action:** You will need to re-configure your settings using the new files generated by v3. Do not just copy-paste your old v2 `config.yml` directly, as the structure has changed. See the [Config Reference](04-config-reference.md) for the new layout.

## 3. The New "Dialog" Prompt (`<d:...>`)
...
## 4. Compound Dialogs (Multiple inputs at once!)
