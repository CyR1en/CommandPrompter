...
import dev.cyr1en.promptpaper.approval.PlayerInteractionLeaseRegistry;
...
/**
 * Manages the lifecycle of interactive prompt sessions for players. Parses command lines for prompt
 * tags, tracks per-player sessions, collects answers, and dispatches post-completion/cancellation
 * commands. All session state is held in a {@link ConcurrentHashMap} keyed by player UUID.
 */
public class PromptEngine {
  ...
  private final AtomicLong sessionIncarnationSequence;
  private final ScreenKeyResolver screenKeyResolver;
  private volatile ExecutionRegistry executionRegistry;
  private volatile PlayerInteractionLeaseRegistry leaseRegistry;
  ...
  public PromptEngine(
      CommandPrompter plugin, Scheduler scheduler, ScreenKeyResolver screenKeyResolver) {
  ...
  public PromptEngine(
    ...
    this.sessionIncarnationSequence = new AtomicLong();
    this.screenKeyResolver =
        screenKeyResolver != null ? screenKeyResolver : defaultScreenKeyResolver(plugin);
  ...
  public void setExecutionRegistry(ExecutionRegistry executionRegistry) {
    this.executionRegistry = executionRegistry;
  }

  public void setLeaseRegistry(PlayerInteractionLeaseRegistry leaseRegistry) {
    this.leaseRegistry =
        leaseRegistry != null ? leaseRegistry : new PlayerInteractionLeaseRegistry();
  }

  public PlayerInteractionLeaseRegistry getEffectiveLeaseRegistry() {
    ...
    return new PlayerInteractionLeaseRegistry();
  }
   ...
   *
   * <p>When {@code Ignore-MiniMessage} is enabled in the config and the prompt delimiters are angle
   * brackets, a {@link MiniMessageTagFilter} is attached so that MiniMessage formatting tags (e.g.
   * {@code <red>}, {@code </red>}) are not treated as prompts.
   */
  private CommandLineParser buildParser(CommandPrompter plugin) {
    ...
    try {
      ...
      if (plugin.getPluginLogger() != null) {
        plugin
            .getPluginLogger()
            .err("Failed to obtain valid parser configuration: " + e.getMessage());
      }
      return this.parser != null ? this.parser : new CommandLineParser();
    }
  }
  ...
  public CommandLineParser prepareParser(CommandPrompterConfig config) {
    ...
    boolean useFilter =
        config.ignoreMiniMessage()
            && "<".equals(parserConfig.opening())
            && ">".equals(parserConfig.closing());
  ...
  public void reloadParser() {
    try {
      ...
      if (plugin != null && plugin.getPluginLogger() != null) {
        plugin
            .getPluginLogger()
            .err("Failed to reload parser; retaining current parser: " + e.getMessage());
      }
    }
  }
   ...
   *
   * <p>The monitor makes the gate transition atomic with the session-creation path. A prompt that
   * is already in the small hand-off between parsing and screen creation either completes that
   * hand-off before the barrier is acquired or is rejected before it can create screen state.
   *
   * @return {@code true} when this caller owns the barrier, or {@code false} when another reload is
   *     already in progress
   */
   ...
   *
   * <p>This uses an existing localized reload failure key because the message catalog is owned by
   * the resource/configuration lane. The raw tagged command is still rejected by the caller.
   */
   ...
   * barrier. This prevents a reload from being acquired between session acceptance and creation of
  ...
  /**
   * Parses a command line and, if it contains prompt tags, starts a new session for the player and
   * returns the parsed result.
   *
   * <p>Compatibility wrapper around {@link #interceptResult(Player, String)}.
   *
   * @return the parsed command with prompts, or empty if no prompts were found (or the command was
   *     rejected)
   */
   ...
   * references, validator aliases, and screen keys fail-closed, and begins a new prompt session if
   * valid prompts are present.
   *
  ...
  public InterceptResult interceptResult(Player player, String commandLine) {
    if (rejectIfReloading(player) || (plugin != null && !plugin.isPluginActive())) {
      return recordLastResult(
          player,
          new InterceptResult.RejectedFailClosed("Configuration reload or shutdown in progress"));
    }
    var config =
        plugin != null && plugin.getConfigLoader() != null
            ? plugin.getConfigLoader().getConfig()
            : null;
    if (config != null && config.enablePermission() && !player.hasPermission("promptpaper.use")) {
      plugin
          .getPluginLogger()
          .debug(
              "Player " + player.getName() + " lacks promptpaper.use, skipping prompt intercept");
      return recordLastResult(player, InterceptResult.RejectedPermission.INSTANCE);
    }
    ...
    try {
      ...
      var safeMsg =
          C0_CONTROLS
              .matcher(e.getMessage() != null ? e.getMessage() : "malformed tag")
              .replaceAll("");
      if (safeMsg.length() > 128) safeMsg = safeMsg.substring(0, 128);
      plugin
          .getPluginLogger()
          .warn("Structural command parse failed for " + player.getName() + ": " + safeMsg);
      return recordLastResult(
          player,
          new InterceptResult.RejectedFailClosed("Structural command parse failed: " + safeMsg));
    }

    var presetSnapshot =
        plugin != null
                && plugin.getPresetRegistry() != null
                && plugin.getPresetRegistry().getSnapshot() != null
    ...
    if (!missingPrompts.isEmpty()
        || !missingPostCmds.isEmpty()
        || !missingValidators.isEmpty()
        || !missingGates.isEmpty()) {
      failFastMissing(player, missingPrompts, missingPostCmds, missingValidators, missingGates);
      return recordLastResult(
          player, new InterceptResult.RejectedFailClosed("Missing presets or validators"));
    }
    ...
    if (unresolvedKey.isPresent()) {
      ...
      return recordLastResult(
          player,
          new InterceptResult.RejectedFailClosed(
              "Unresolved screen key: " + sanitizeKey(unresolvedKey.get())));
    }
    ...
    if (!parsed.hasPrompts()) {
      if (parsed.hasGates()) {
        ...
        if (plugin.getConfigLoader() != null && plugin.getConfigLoader().getI18n() != null) {
          player.sendMessage(
              plugin.getConfigLoader().getI18n().get("command.error.missing_preset", player));
        }
        return recordLastResult(
            player,
            new InterceptResult.RejectedFailClosed(
      ...
      return recordLastResult(player, InterceptResult.NoPrompts.INSTANCE);
    }
    ...
    if (hasActiveSession(player) || hasActiveExecution(player) || hasActiveApprovalLease(player)) {
      plugin
          ...
          .debug(
              "Player "
                  + player.getName()
                  + " already has an active session or execution, aborting new session");
      if (plugin.getConfigLoader() != null && plugin.getConfigLoader().getI18n() != null) {
        player.sendMessage(
            plugin.getConfigLoader().getI18n().get("prompt.error.session_active", player));
      }
    ...
    var planDefinition = ExecutionPlanAdapter.fromParsedCommand(effectiveParsed, templateSyntax);

    long candidateIncarnation = sessionIncarnationSequence.incrementAndGet();
    var claimOpt =
        getEffectiveLeaseRegistry().acquirePrompt(player.getUniqueId(), candidateIncarnation);
    if (claimOpt.isEmpty()) {
      plugin
          .getPluginLogger()
          .debug(
    ...
    synchronized (sessionLifecycleMonitor) {
      ...
      if (!rejectedByReload && !hasActiveExecution(player)) {
        sessions.compute(
            player.getUniqueId(),
            (uuid, existing) -> {
    ...
    if (rejectedByReload) {
      ...
      return recordLastResult(
          player, new InterceptResult.RejectedFailClosed("Configuration reload in progress"));
    }
    if (!accepted.get()) {
      getEffectiveLeaseRegistry().releasePromptIfExact(player.getUniqueId(), candidateIncarnation);
      plugin
    ...
    plugin
        ...
        .debug(
            "Intercepted "
                + effectiveParsed.promptTags().size()
                + " prompts for "
                + player.getName());
    return recordLastResult(player, new InterceptResult.Started(effectiveParsed));
  }

  private InterceptResult recordLastResult(Player player, InterceptResult result) {
  ...
  ParsedCommand applyPresetSanitize(ParsedCommand parsed) {
    var snapshot =
        plugin != null && plugin.getPresetRegistry() != null
            ? plugin.getPresetRegistry().getSnapshot()
            : null;
    return applyPresetSanitize(parsed, snapshot);
  }

  ParsedCommand applyPresetSanitize(ParsedCommand parsed, PresetSnapshot snapshot) {
  ...
  /**
   * Whether the given command line contains at least one tag (prompt or PCM). Useful for callers
   * that need to distinguish "no tag form at all" from "had tag form but parsing returned empty for
   * some reason" — for example, the fail-fast path that must cancel the underlying command
   * dispatch.
   */
  ...
  /**
   * Whether the given command line contains at least one <b>preset reference</b> ({@code <@id>} or
   * {@code <!@id>}) or approval gate. The listener uses this to decide whether to cancel the {@link
   * org.bukkit.event.player.PlayerCommandPreprocessEvent} even when no prompt session was started —
   * the literal preset or gate tag must never reach the underlying command dispatcher.
   */
  ...
  /**
   * Whether parsing the given command line produces a structural parser error (e.g. invalid
   * timeout, exceeded tag limits).
   */
  ...
  public Optional<SessionResult> submit(Player player, String answer) {
    return updateSession(
        player, session -> session.submitAnswer(answer), "submit", "Answer accepted");
  }

  /**
   * Submit a batch of answers to the current prompt. The current prompt must be a compound tag with
   * the matching number of sub-answers. See {@link PromptSession#submitAnswers(List)} for the
   * size-validation rules.
   */
  public Optional<SessionResult> submitAnswers(Player player, List<String> answers) {
    return updateSession(
        player,
        session -> session.submitAnswers(answers),
        "submitAnswers",
        "Compound answers accepted");
  }

  /**
   * Submit a batch of answers to the current prompt with an explicit expected answer count. This is
   * the arity-aware entry point for dialog flows whose effective answer count is not derivable from
   * the parsed tag shape — JSON dialog presets may submit 0, 1, or N answers. Completion/finish
   * behavior mirrors {@link #submit(Player, String)}. See {@link PromptSession#submitAnswers(List,
   * int)} for the validation rules.
   *
   * @return the completed session result if all prompts are now answered, or empty
   */
  public Optional<SessionResult> submitAnswers(
      Player player, List<String> answers, int expectedCount) {
    return updateSession(
        player,
        session -> session.submitAnswers(answers, expectedCount),
        "submitAnswers(expected=" + expectedCount + ")",
        "Dialog answers accepted");
  }

  private Optional<SessionResult> updateSession(
      Player player,
      UnaryOperator<PromptSession> update,
      String operation,
    ...
    if (!found.get()) {
      plugin
          .getPluginLogger()
          .debug("No active session for " + player.getName() + " on " + operation);
    ...
    plugin
        ...
        .debug(
            acceptedMessage
                ...
                + player.getName()
                + ", "
                + (session != null ? session.remainingCount() : 0)
                + " remaining");
    return Optional.empty();
  }
    ...
    synchronized (this) {
      if (executionCoordinator == null) {
        ...
        this.executionCoordinator =
            new ExecutionCoordinator(plugin, this, reg, primaryDisp, actionDisp);
      }
  ...
  public void cancel(Player player, CancelReason reason, DispatchContext dispatchContext) {
    cancel(
        player,
        reason,
        dispatchContext != null ? dispatchContext.toSnapshot() : null,
        CancellationMode.USER_ACTIONS);
  }

  /** Atomically cancels a session and dispatches its cancellation plan via ExecutionCoordinator. */
  public void cancel(
      Player player, CancelReason reason, DispatchContextSnapshot dispatchContextSnapshot) {
    cancel(player, reason, dispatchContextSnapshot, CancellationMode.USER_ACTIONS);
  }

  public void cancel(
      Player player, CancelReason reason, DispatchContext dispatchContext, CancellationMode mode) {
  ...
  public void cancel(
      Player player,
      CancelReason reason,
      DispatchContextSnapshot dispatchContextSnapshot,
    ...
    if (mode == CancellationMode.DISCARD_ONLY
        || isReloadInProgress()
        || (plugin != null && !plugin.isPluginActive())) {
      var cancelledIncarnation = new AtomicLong(-1L);
      sessions.compute(
          uuid,
          (key, existing) -> {
            if (existing != null) {
    ...
    var cancelledIncarnation = new AtomicLong(-1L);
    var cancelledGeneration = new AtomicLong(-1L);
    sessions.compute(
        player.getUniqueId(),
        (u, session) -> {
          if (session == null || !session.isActive()) return session;
    ...
    plugin
        .getPluginLogger()
        .debug("Session cancelled for " + player.getName() + " reason=" + reason);
    ...
    if (artifactsOpt.isEmpty()) {
      var safePlayer =
          player != null ? C0_CONTROLS.matcher(player.getName()).replaceAll("") : "unknown";
      if (safePlayer.length() > 64) safePlayer = safePlayer.substring(0, 64);
      plugin
          .getPluginLogger()
          .warn(
              "Missing or mismatched inception artifacts for cancelled session of "
                  + safePlayer
                  + " (inc="
                  + inc
                  + ", gen="
                  + gen
                  + "); failing closed with no on-cancel PCM dispatch");
      return;
    }
  ...
  public void cancelAll(CancellationMode mode) {
    var snapshot = new ArrayList<>(sessions.keySet());
          ...
          var task =
              player
                  ...
                  .run(
                      plugin,
                      scheduledTask ->
                          cancel(player, CancelReason.MANUAL, (DispatchContextSnapshot) null, mode),
                      () -> discard(uuid));
  ...
  public void discardAll() {
    for (var entry : sessions.entrySet()) {
      getEffectiveLeaseRegistry()
          .releasePromptIfExact(entry.getKey(), entry.getValue().incarnation());
    }
    sessions.clear();
  ...
  /**
   * Atomically takes and removes the captured session inception artifacts for the given player
   * UUID, verifying that the stored artifacts match the expected incarnation.
   *
  ...
  public Optional<SessionInceptionArtifacts> takeInceptionArtifacts(
      UUID uuid, long expectedIncarnation) {
    return takeInceptionArtifacts(uuid, expectedIncarnation, -1L);
  }

  /**
   * Atomically takes and removes the captured session inception artifacts for the given player
   * UUID, verifying that the stored artifacts match the expected incarnation and generation.
   *
   * @param uuid player UUID
   * @param expectedIncarnation expected session incarnation
  ...
  public Optional<SessionInceptionArtifacts> takeInceptionArtifacts(
    ...
    sessionInceptionArtifacts.compute(
        uuid,
        (key, existing) -> {
          if (existing != null
  ...
  /**
   * Looks up the captured session inception artifacts for the given player UUID and expected
   * incarnation without removing.
   *
   * @param uuid player UUID
   * @param expectedIncarnation expected session incarnation
   * @return optional containing the inception artifacts, or empty if absent or mismatched
  ...
  /**
   * Looks up the captured session inception artifacts for the given player UUID, expected
   * incarnation, and expected generation without removing.
   *
   * @param uuid player UUID
   * @param expectedIncarnation expected session incarnation
   * @param expectedGeneration expected session generation (or negative to ignore generation)
  ...
  /**
   * Returns the list of preset prompt ids that appear in {@code parsed} but are not registered in
   * the plugin's {@code PresetRegistry}. Order matches the order of occurrence in the parsed
   * command.
   */
  private List<String> findMissingPromptPresets(ParsedCommand parsed, PresetSnapshot snapshot) {
    var registry = plugin != null ? plugin.getPresetRegistry() : null;
  ...
  /**
   * Returns the list of preset post-command ids that appear in {@code parsed} but are not
   * registered in the plugin's {@code PresetRegistry}. Order matches the order of occurrence in the
   * parsed command.
   */
  private List<String> findMissingPostCommandPresets(
      ParsedCommand parsed, PresetSnapshot snapshot) {
    var registry = plugin != null ? plugin.getPresetRegistry() : null;
    if (registry == null && snapshot == null) {
  ...
  private List<String> findMissingGatePresets(ParsedCommand parsed, PresetSnapshot snapshot) {
    if (parsed == null
        || parsed.preDispatchGates() == null
        || parsed.preDispatchGates().isEmpty()) {
      return List.of();
    }
    var missing = new ArrayList<String>();
    for (var gate : parsed.preDispatchGates()) {
      if (gate instanceof PreDispatchGateSpec.Approval approval) {
  ...
  /**
   * Returns the list of validator aliases that appear in {@code parsed} but are not configured in
   * {@code PromptConfig}. Order matches occurrence order.
   */
  ...
  private void checkMissingValidator(String alias, PromptConfig config, List<String> missing) {
  ...
  /**
   * Logs a severe warning to the console and sends a localized error message to the player when a
   * command references one or more unknown preset ids or validator aliases. Per the spec, the
  ...
  private void failFastMissing(
    ...
    plugin
        ...
        .err(
            "Fail-fast: command from "
                + playerUuid
                + " references unknown element(s) ["
                + summary
                + "] — command NOT executed.");
    var i18n = plugin.getConfigLoader().getI18n();
    if (!missingValidators.isEmpty()
        && missingPrompts.isEmpty()
        && missingPostCmds.isEmpty()
        && missingGates.isEmpty()) {
