...
/**
 * Base class for prompt screens that delegate to a {@link ScreenProvider} NMS implementation,
 * falling back to a chat prompt if no provider is available.
 */
public abstract class AbstractWrapperPromptScreen implements InputScreen {
  ...
  protected AbstractWrapperPromptScreen(
      CommandPrompter plugin, Player player, String displayText, List<ScreenProvider> providers) {
    this.plugin = plugin;
    this.player = player;
    ...
    var fallback =
        new ChatPromptScreen(
            ...
            new dev.cyr1en.promptpaper.preset.ChatPrompt(
                ...
                true));
    fallback.onResult(this::handleFallbackResult);
    return fallback;
  }
