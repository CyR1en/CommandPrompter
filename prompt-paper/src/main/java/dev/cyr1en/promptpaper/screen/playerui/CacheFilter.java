...
/**
 * Base class for player-list filters used by {@link HeadCache} to narrow which player heads appear
 * in a {@link PlayerUIScreen}.
 */
public abstract class CacheFilter {
  ...
  public static class WorldFilter extends CacheFilter {
    public WorldFilter() {
      super(Pattern.compile("w"), "World");
    }

    @Override
    public List<Player> filter(Player p) {
      return p.getWorld().getPlayers();
    }
  ...
  public static class RadialFilter extends CacheFilter {
    ...
    public RadialFilter(int radius) {
      super(Pattern.compile("r(\\d+)"), "Radial", 1);
      this.radius = radius;
    }
    ...
    public List<Player> filter(Player p) {
      return p.getWorld().getPlayers().stream()
          .filter(o -> o.getLocation().distance(p.getLocation()) <= radius)
          .toList();
    }
  }
