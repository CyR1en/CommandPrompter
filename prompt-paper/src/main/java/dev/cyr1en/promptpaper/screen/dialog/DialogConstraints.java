...
/**
 * Per-tag input constraints parsed from a {@code PromptTag} filter slot, merged with {@link
 * DialogConfig} defaults. Each kind (TEXT, NUMBER, CHOICE, TAB) has its own subset of fields;
 * unused fields are zeroed.
 */
public record DialogConstraints(
    DialogInputKind kind,
    String rawFilter,
    /* text-only */ int maxLength,
    boolean multiline,
    int multilineMaxLines,
    int width,
    /* choice-only */ List<String> options,
    /* number-only */ float min,
    float max,
    float step,
    float initial,
    /* tab-only */ Integer maxButtons) {
  /**
   * Parses a raw filter string into constraints, falling back to the given {@link DialogConfig}
   * defaults for any unspecified values.
   */
  public static DialogConstraints from(String filter, DialogConfig defaults) {
    ...
    var bracketContent = bracketIdx >= 0 ? extractBracket(filter.substring(bracketIdx)) : "";
  ...
  private static DialogConstraints parseTitle(String bracket, DialogConfig d) {
    return new DialogConstraints(
        DialogInputKind.TITLE, bracket, 0, false, 0, 200, List.of(), 0f, 0f, 0f, 0f, null);
  }
  ...
  private static DialogConstraints parseBody(String bracket, DialogConfig d) {
    ...
    return new DialogConstraints(
        DialogInputKind.BODY, type, 0, false, 0, width, List.of(), 0f, 0f, 0f, 0f, null);
  }
      ...
      if (bracket.contains("=")) {
        ...
        for (var part : parts) {
          ...
          if (p.startsWith("max_length=")) {
            try {
              maxLength = clampInt(Integer.parseInt(p.substring(11)), 1, 8192);
            } catch (NumberFormatException ignored) {
            }
          } else if (p.startsWith("max_lines=")) {
            try {
              maxLines = clampInt(Integer.parseInt(p.substring(10)), 1, 8192);
            } catch (NumberFormatException ignored) {
            }
          } else if (p.startsWith("width=")) {
            try {
              width = clampInt(Integer.parseInt(p.substring(6)), 1, 8192);
            } catch (NumberFormatException ignored) {
            }
          }
        }
        ...
        if (parts.length >= 1) {
          try {
            maxLength = clampInt(Integer.parseInt(parts[0].trim()), 1, 8192);
          } catch (NumberFormatException ignored) {
          }
        }
        if (parts.length >= 2) {
          try {
            maxLines = clampInt(Integer.parseInt(parts[1].trim()), 1, 8192);
          } catch (NumberFormatException ignored) {
          }
        }
        if (parts.length >= 3) {
          try {
            width = clampInt(Integer.parseInt(parts[2].trim()), 1, 8192);
          } catch (NumberFormatException ignored) {
          }
        }
      }
  ...
  private static DialogConstraints parseChoice(String bracket, DialogConfig d) {
    ...
    return new DialogConstraints(
        DialogInputKind.CHOICE, bracket, 0, false, 0, 200, options, 0f, 0f, 0f, 0f, null);
  }
  ...
  private static DialogConstraints parseTab(String bracket, DialogConfig d) {
    ...
    return new DialogConstraints(
        DialogInputKind.TAB, bracket, 0, false, 0, 200, List.of(), 0f, 0f, 0f, 0f, maxButtons);
  }
