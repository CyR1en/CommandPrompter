...
/**
 * Looks up tab-completion suggestions for a {@code d:tab} prompt by delegating to the server's
 * Bukkit {@link org.bukkit.command.CommandMap}. Returns an empty list on any failure (null input,
 * no match, no permission).
 */
public final class TabCompletionService {

  /**
   * Returns completions for the last token of the given command line, or an empty list if the input
   * is invalid or the command is unknown.
   */
