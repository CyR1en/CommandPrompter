...
/**
 * Prompt screen that sends a chat message and captures the player's next chat input as the answer.
 */
public class ChatPromptScreen implements InputScreen {
  ...
  public ChatPromptScreen(
      CommandPrompter plugin, Player player, dev.cyr1en.promptpaper.preset.ChatPrompt chatPrompt) {
    this.plugin = plugin;
    this.player = player;
  ...
  public void open() {
    ...
    plugin
        .getPluginLogger()
        .debug("Opening chat prompt for " + player.getName() + " text=" + displayText);
    ...
    if (chatPrompt.cancel() != null && !chatPrompt.id().startsWith("inline-")) {
      ...
      if (chatPrompt.cancel().hoverMessage() != null
          && !chatPrompt.cancel().hoverMessage().isBlank()) {
    ...
    if (sendCancel) {
      ...
      if (isClickable) {
        builder =
            builder
                .clickEvent(ClickEvent.runCommand("/cmdp cancel"))
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      var lineComponent =
          Objects.isNull(lineCancel)
              ? ComponentUtil.mini(linePrefix + line)
