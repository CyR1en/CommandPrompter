...
/**
 * Fail-closed token generation for item prompt answers. Every token passes through {@link
 * ItemTokenFormatter}; invalid, oversized, or otherwise unformattable values throw {@link
 * IllegalArgumentException} so the screen can cancel with {@link
 * dev.cyr1en.promptcore.CancelReason#ERROR} instead of emitting a bad answer.
 */
public final class ItemAnswerTokens {
  ...
  public static String fromSelection(
      ItemOutputFormat format, ItemSnapshot snapshot, ItemStack verifiedItem) {
    Objects.requireNonNull(format, "format must not be null");
    Objects.requireNonNull(snapshot, "snapshot must not be null");
    Objects.requireNonNull(verifiedItem, "verifiedItem must not be null");
    if (snapshot.isEmpty()
        || verifiedItem.isEmpty()
        || verifiedItem.getType().isAir()
        || verifiedItem.getAmount() <= 0) {
  ...
  /**
   * Generates the answer token for a catalog selection. Catalog entries are pre-validated by the
   * catalog parser, so failures here indicate a registry contract violation.
   *
  ...
  public static String fromCatalogEntry(ItemOutputFormat format, CatalogEntry entry) {
    ...
    return switch (format) {
      ...
      case SLOT ->
          throw new IllegalArgumentException("Slot output is not supported for catalog selections");
    };
  }
}
