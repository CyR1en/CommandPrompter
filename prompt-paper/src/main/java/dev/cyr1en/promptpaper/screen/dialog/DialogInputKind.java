...
/**
 * The kind of input field a {@code <d:...>} prompt renders, driven by the filter slot of a {@code
 * PromptTag}. Unknown or empty filters resolve to {@link #TEXT}.
 */
public enum DialogInputKind {
  ...
  /**
   * Parses a raw filter string into a kind, stripping trailing bracket constraints. Accepts {@code
   * "num"}, {@code "choice"}, {@code "tab"} (case-insensitive); unknown/empty inputs return {@link
   * #TEXT}.
   */
  ...
  /**
   * Whether this kind renders an input widget whose value becomes a submitted answer. Only {@link
   * #TITLE} and {@link #BODY} (layout-only rows) are not answer-bearing.
   */
