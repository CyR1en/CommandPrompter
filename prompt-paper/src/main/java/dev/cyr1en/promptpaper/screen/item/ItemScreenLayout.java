 ...
 *
 * <ul>
 *   <li>{@code 0..26} — main storage snapshots (player slots 9..35)
 *   <li>{@code 27..35} — hotbar snapshots (player slots 0..8)
 *   <li>{@code 36..44} — divider row
 *   <li>{@code 45} offhand, {@code 46} boots, {@code 47} leggings, {@code 48} chest, {@code 49}
 *       helmet
 *   <li>{@code 50} info, {@code 51..52} filler, {@code 53} cancel
 * </ul>
 *
 * <p>Hand mode (9 slots): {@code 4} captured main hand, {@code 8} cancel, rest filler.
 *
 * <p>Catalog mode (54 slots): {@code 0..44} page content, {@code 45} previous page, {@code 49} page
 * info, {@code 52} next page, {@code 53} cancel.
 */
public final class ItemScreenLayout {
  ...
  /**
   * Whether the given GUI slot represents a selectable entry for the mode. Cancel and navigation
  ...
  public static ItemStack infoItem(String header, Component promptText) {
    var item =
        named(Material.PAPER, ComponentUtil.mini("<!italic><yellow>" + header + "</yellow>"));
  ...
  public static ItemStack pageInfoItem(int page, int pageCount, int totalEntries) {
    ...
    var item =
        named(
            Material.PAPER,
            ComponentUtil.mini(
                "<!italic><aqua>Page " + (page + 1) + " of " + pageCount + "</aqua>"));
    var meta = item.getItemMeta();
    if (meta != null) {
