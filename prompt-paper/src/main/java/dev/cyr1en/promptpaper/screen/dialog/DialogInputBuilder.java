...
/**
 * Static factory helpers for {@link DialogInput} instances used by {@link
 * dev.cyr1en.promptpaper.screen.DialogPromptScreen}. Internal API.
 */
public final class DialogInputBuilder {
  ...
  /**
   * The default single-input key. For compound dialogs each row gets its own key derived from this
   * prefix (e.g. {@code answer_0}, {@code answer_1}).
   */
  public static final String DEFAULT_INPUT_KEY = "answer";

  /**
   * Reserved input key used for tab-completion fallback text inputs to avoid colliding with row
   * keys (such as {@code "answer"} for row 0).
   */
  ...
  public static DialogInput buildText(DialogConstraints c, Component label, String key) {
    var builder =
        DialogInput.text(key, label).width(c.width()).labelVisible(true).maxLength(c.maxLength());
    if (c.multiline()) {
      builder =
          builder.multiline(TextDialogInput.MultilineOptions.create(c.multilineMaxLines(), null));
    }
   ...
   *
   * <p>Option labels from {@link DialogConstraints#options()} are used as both the option's display
   * text and its id — the id is what {@code DialogResponseView.getText(key)} returns. The first
   * option is initially selected; only one option may carry the {@code initial} flag at a time, and
   * a single-option input with no initially selected option is rejected by the client, so we always
   * force the first option to be the default.
   */
