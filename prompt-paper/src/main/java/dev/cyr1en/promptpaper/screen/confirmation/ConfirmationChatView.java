 ...
 *
 * <p>Renders interactive Adventure components with clickable confirm and decline buttons bound to
 * single-use cryptographic nonces. All player messages are dispatched via the player entity
 * scheduler.
 */
public class ConfirmationChatView implements ConfirmationView, Listener {
  ...
  public static final Component DEFAULT_CONFIRM_LABEL =
      ComponentUtil.mini("<green>[Confirm]</green>");
   ...
   *
   * <p>Maps {@link ConfirmationDecision#CONFIRM} to {@link ConfirmationOutcome.Confirmed} and
   * {@link ConfirmationDecision#DECLINE} to {@link ConfirmationOutcome.Declined} exactly once,
   * guarding against duplicate delivery.
   *
  ...
  public Component renderMessage(String nonce) {
    ...
    if (confirmBtn.hoverEvent() == null) {
      confirmBtn =
          confirmBtn.hoverEvent(
              HoverEvent.showText(ComponentUtil.mini("<green>Click to confirm</green>")));
    }

    var declineBtn = cancelLabel.clickEvent(ClickEvent.runCommand(declineCmd));
    if (declineBtn.hoverEvent() == null) {
      declineBtn =
          declineBtn.hoverEvent(
              HoverEvent.showText(ComponentUtil.mini("<red>Click to decline</red>")));
    }
