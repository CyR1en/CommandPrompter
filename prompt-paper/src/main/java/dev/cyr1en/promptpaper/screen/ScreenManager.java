...
/**
 * Orchestrates prompt sessions by routing tags to the appropriate screen type, collecting answers,
 * and dispatching the assembled command.
 */
public class ScreenManager {
  ...
  private static final java.util.regex.Pattern C0_CONTROLS =
      java.util.regex.Pattern.compile("[\\u0000-\\u001F\\u007F]");

  public ScreenManager(
      CommandPrompter plugin, PromptEngine engine, PromptFactory factory, Scheduler scheduler) {
  ...
  public ScreenManager(
    ...
    this.screenCreator =
        screenCreator != null ? screenCreator : (factory != null ? factory::createFromTag : null);
    ...
    dev.cyr1en.promptpaper.custom.CustomScreenAuditLogger effectiveAuditLogger =
        auditLogger != null
            ? auditLogger
    ...
    this.providerLifecycleCoordinator =
        new ProviderLifecycleCoordinator(
  ...
  public boolean verifyAttempt(SessionVerificationSnapshot snapshot) {
    ...
    if (snapshot.promptIndex() >= 0 && session.currentIndex() != snapshot.promptIndex())
      return false;
    var activeHandle = activeScreenHandles.get(snapshot.playerUuid());
    if (activeHandle == null || activeHandle.attemptToken() != snapshot.attemptToken())
      return false;
    return true;
  }

  /**
   * Intercepts the command line for prompt tags and begins showing the first prompt to the player.
   */
  ...
  /**
   * Like {@link #startSession} but with a dispatch mode and optional permission key for
   * post-session command execution.
   */
  public void startDelegatedSession(
      ...
      var task =
          target
              ...
              .run(
                  plugin,
                  scheduledTask ->
                      startDelegatedSessionOnPlayer(target, commandLine, mode, permissionKey),
                  () -> discardState(uuid));
      if (task == null) discardState(uuid);
    } catch (Throwable t) {
      plugin
          .getPluginLogger()
          .err("Unable to schedule delegated session for " + uuid + ": " + t.getMessage());
      discardState(uuid);
    }
  }
  ...
  private void startDelegatedSessionOnPlayer(
    ...
    try {
      plugin
          .getPluginLogger()
          .debug(
              "Delegated session: target="
                  + target.getName()
                  + " mode="
                  + mode
                  + " permKey="
                  + permissionKey);
      var normalized = commandLine.startsWith("/") ? commandLine.substring(1) : commandLine;
      ...
      if (parsed.isEmpty()) {
        if (!engine.commandHasTagForm(normalized)) {
          ...
          plugin
              .getPluginLogger()
              .debug(
                  "Command had tag form but intercept rejected it "
                      + "(missing preset, no permission, or active session); not dispatching");
        }
        return;
      }
      var started =
          engine.runIfReloadNotInProgress(
              () -> {
      ...
      plugin.getPluginLogger().err("Delegated session failed for " + uuid + ": " + t.getMessage());
      discardState(uuid);
    }
  }
  ...
  private void dispatchDirect(
    ...
    var permissionSnapshot =
        mode == DispatchMode.ATTACHMENT
            ? capturePermissionSnapshot(permissionKey)
            : List.<String>of();
    var dispatchSnapshot =
        switch (mode) {
    ...
    var plan =
        dev.cyr1en.promptcore.plan.ExecutionPlanAdapter.fromParsedCommand(parsed, templateSyntax);
    var sessionResult =
        new dev.cyr1en.promptcore.SessionResult(commandLine, List.of(), List.of(), List.of());
    var completion =
        InputCompletion.of(uuid, 1L, 0L, sessionResult, plan, snapshot, dispatchSnapshot);
    executionCoordinator.coordinate(target, completion, sessionResult);
  }

  /**
   * Advances the session to the next prompt tag and displays it, or completes the session if all
      ...
      plugin
          ...
          .warn(
              "Player "
                  ...
                  + tag.rawTag());
      player.sendMessage(
          plugin.getConfigLoader().getI18n().get("prompt.error.invalid_title_filter", player));
      cancelAll(player);
      return;
    }
  ...
  private void showPrompt(Player player, PromptTag tag) {
    ...
    try {
      if (resolution instanceof ScreenResolution.Custom customRes) {
          ...
          plugin
              ...
              .warn(
                  "Custom screen provider '"
                      + customHandle.key()
                      + "' is no longer active for player "
                      + player.getName()
                      + "; failing closed");
          teardown(player, CancelReason.ERROR, false, false);
          return;
        ...
        var screenContext =
            new dev.cyr1en.promptui.api.ScreenContext(
                customHandle.key(), tag.displayText(), tag.flags(), tag.sanitize());

        var snapshot =
            new SessionVerificationSnapshot(
                uuid, expectedIncarnation, expectedGeneration, currentPromptIndex, attemptToken);
        ...
        var adapter =
            CustomScreenAdapter.lazy(
                ...
                () ->
                    customHandleRef
                        .invokeFactory(
                            promptFactory -> promptFactory.createScreen(player, screenContext))
                        .orElse(null),
                playerExecutor,
                snapshot,
                this::verifyAttempt,
      ...
      if (!linked) {
        plugin
            ...
            .warn(
                "Custom screen provider '"
                    + (customHandle != null ? customHandle.key() : tag.key())
                    + "' became inactive during screen linking for player "
                    + player.getName()
                    + "; failing closed");
      ...
      screen.onResult(
          result ->
              handleResult(player, result, expectedIncarnation, expectedGeneration, attemptToken));
      ...
      plugin.getPluginLogger().err("Unable to open prompt screen for " + uuid + ": " + safeMsg);
  ...
  /**
   * Builds a {@link DialogCompletionContext} for TAB prompts (both inline {@code d:tab} and preset
   * dialogs with {@code actions_source: "tab_completion"}) by reconstructing the partial command
   * from the session's parsed command and current answers. Returns null for non-TAB prompts.
   */
  ...
  /**
   * Processes a screen result: validates the answer, handles compound payloads, and either advances
   * the session or dispatches the command.
   *
   * <p>Dialog screens (inline compound, inline single, or JSON preset) are detected and unwrapped
   * before they are removed from the active-screens map and their result is routed through one
   * arity-aware batch handler, regardless of whether the parsed tag is compound.
   */
  ...
  private void handleResult(
    ...
    try {
      ...
      plugin
          .getPluginLogger()
          .debug(
              "Unable to schedule handleResult for "
                  + player.getUniqueId()
                  + ": "
                  + t.getMessage());
      discardState(player.getUniqueId());
  ...
  private void handleOpenFailure(
    ...
    try {
      ...
      plugin
          .getPluginLogger()
          .debug("Unable to schedule handleOpenFailure for " + uuid + ": " + safeMsg);
      discardState(uuid);
    }
  }
  ...
  private void handleOpenFailureOnPlayerScheduler(
    ...
    if (!session.isActive()
      ...
      plugin
          .getPluginLogger()
          .debug(
              "Discarding stale open failure for "
                  + player.getName()
                  + " (expected inc="
                  + expectedIncarnation
                  + ", gen="
    ...
    if (attemptToken > 0
        && (currentAttempt == null || currentAttempt.longValue() != attemptToken)) {
      plugin
          .getPluginLogger()
          .debug(
              "Discarding stale screen attempt on open failure for "
                  + player.getName()
                  + " (expected attempt="
                  + attemptToken
                  + ", current attempt="
    ...
    if (error != null) {
      ...
      errorDetail =
          ": "
              + (msg != null
                  ? C0_CONTROLS.matcher(msg).replaceAll("")
  ...
  private void handleResultOnPlayerScheduler(
    ...
    if (attemptToken > 0
      ...
      plugin
          .getPluginLogger()
          .debug(
              "Discarding stale screen attempt for "
                  + player.getName()
                  + " (expected attempt="
                  + attemptToken
                  + ", current attempt="
    ...
    if (confirmationScreen != null) {
      handleConfirmationResult(
          player, confirmationScreen, result, expectedIncarnation, expectedGeneration);
      return;
    }
    ...
    if (dialogScreen != null) {
      handleDialogResult(
          player, tag, result, dialogScreen, expectedIncarnation, expectedGeneration);
      return;
    }
    ...
    var cleanAnswer =
        C0_CONTROLS.matcher(result.answer() != null ? result.answer() : "").replaceAll("");
  ...
  public static dev.cyr1en.promptpaper.screen.confirmation.ConfirmationPromptScreen
      unwrapConfirmationScreen(InputScreen screen) {
    if (screen instanceof TitleWrapperScreen wrapper) {
    ...
    return screen
            instanceof
            dev.cyr1en.promptpaper.screen.confirmation.ConfirmationPromptScreen confirmation
        ? confirmation
        : null;
  }
  ...
  private void handleConfirmationResult(
                ...
                () -> {
                  if (result.cancelled()) {
                    return dev.cyr1en.promptpaper.screen.confirmation.ConfirmationOutcome.cancelled(
                        result.cancelReason() != null
                            ? result.cancelReason()
                            : CancelReason.GUI_EXIT);
                  }
    ...
    plugin
        .getPluginLogger()
        .debug(
            "Confirmation outcome for "
                + player.getName()
                + ": "
                + outcome.getClass().getSimpleName()
                + " (valueMode="
                + confirmationScreen.isValueMode()
                + ")");

    var tagOpt =
        engine
            .getSession(player)
            .flatMap(dev.cyr1en.promptcore.session.PromptSession::currentPrompt);
  ...
  private void handleSubmitted(
    ...
    if (submitted.isPresent()) {
      ...
      var artifactsOpt =
          engine.takeInceptionArtifacts(uuid, expectedIncarnation, expectedGeneration);
      if (artifactsOpt.isEmpty()) {
        ...
        plugin
            .getPluginLogger()
            .err(
                "Missing or mismatched inception artifacts for session completion of "
                    + safeName
                    + " (expected inc="
                    + expectedIncarnation
                    + ", gen="
        ...
        if (plugin.getConfigLoader() != null && plugin.getConfigLoader().getI18n() != null) {
          try {
            player.sendMessage(
                plugin.getConfigLoader().getI18n().get("prompt.error.command_failed", player));
          } catch (Throwable ignored) {
          }
        }
        return;
  ...
  /**
   * Unwraps a {@link TitleWrapperScreen} (if any) and returns the underlying screen when it is a
   * {@link DialogScreen}; otherwise {@code null}. Works through the loadable UI-API marker so the
   * Paper-bound dialog screen class is never referenced here.
   */
  ...
  /**
   * Routes a {@link DialogScreen} result through the arity-aware batch path. The payload is decoded
   * with the screen's effective answer count (cached at open time — never recomputed from tab
   * completion here), each answer is validated against the block-level constraints, and the batch
   * is submitted with that expected count.
   */
  private void handleDialogResult(
    ...
    if (rawAnswers == null) {
      ...
      plugin
          .getPluginLogger()
          .warn("Malformed dialog payload received from dialog for " + player.getName());
      showPrompt(player, tag);
      return;
    }
    var answers =
    ...
    for (var i = 0; i < answers.size(); i++) {
      if (!validateAnswer(player, answers.get(i), tag)) {
        plugin
            .getPluginLogger()
            .debug(
                "Validation failed for answer " + i + " of dialog prompt for " + player.getName());
        showPrompt(player, tag);
        return;
      }
    }
  ...
  /**
   * Decodes a compound RS/US payload into sub-answers, validates each against the block-level
   * constraints, and submits all at once. Used only as a defensive fallback for compound tags that
   * did not produce a {@link DialogPromptScreen}; dialog screens route through {@link
   * #handleDialogResult}. TITLE/BODY layout rows never validate or submit — only answer-bearing
   * sub-tags occupy answer positions.
   */
  private void handleCompoundResult(
    ...
    if (rawAnswers == null) {
      ...
      plugin
          .getPluginLogger()
          .warn("Malformed compound payload received from dialog for " + player.getName());
      showPrompt(player, tag);
      return;
    }
    var answers =
    ...
    for (var i = 0; i < answers.size(); i++) {
      if (!validateAnswer(player, answers.get(i), tag)) {
        plugin
            .getPluginLogger()
            .debug(
                "Validation failed for sub-answer "
                    + i
                    + " of compound prompt for "
                    + player.getName());
        showPrompt(player, tag);
        return;
      }
  ...
  /**
   * Evaluates the current prompt tag's breakIf condition against the combined accepted and
   * candidate answers before submission. If the condition is met (true), cancels the session with
   * MANUAL reason. If an evaluation error occurs, fails closed with ERROR reason.
   *
  ...
  private boolean checkBreakIf(Player player, PromptTag tag, List<String> candidateAnswers) {
    ...
    var combinedAnswers =
        new java.util.ArrayList<String>(existingAnswers.size() + candidateAnswers.size());
    ...
    try {
      ...
      if (shouldBreak) {
        plugin
            .getPluginLogger()
            .debug("BreakIf condition met for " + player.getName() + ": " + tag.breakIf().source());
        teardown(player, CancelReason.MANUAL, false, true);
        return true;
      }
      return false;
   ...
   *
   * <p>For compound tags this is the sub-tag list filtered to answer-bearing kinds (everything
   * except {@link DialogInputKind#TITLE} and {@link DialogInputKind#BODY} — layout rows never
   * validate or enter answer history). Non-compound tags (including JSON preset references) yield
   * the tag itself, so validation falls back to the block-level constraints exactly as before.
   */
  ...
  /**
   * Decodes a compound payload via {@link AnswerEncoding#decode}. Returns null if the payload is
   * malformed.
   */
  ...
  /** Validates a single answer against the tag's type constraint and custom validator alias. */
  private boolean validateAnswer(Player player, String answer, PromptTag tag) {
    ...
    if (answer.length() > maxLen) {
      ...
      player.sendMessage(
          i18n.get(
              "validation.answer_too_long", player, Placeholder.of("max", String.valueOf(maxLen))));
      return false;
    }
    switch (tag.type()) {
      case INTEGER -> {
        try {
          ...
          plugin.getPluginLogger().debug("Integer validation failed for " + player.getName());
      ...
      case STRING -> {
        if (answer.isBlank()) {
          plugin
              .getPluginLogger()
              .debug("String validation failed (blank) for " + player.getName());
  ...
  public boolean hasChatScreen(Player player) {
    return getChatScreen(player) != null;
  }

  private ChatPromptScreen getChatScreen(Player player) {
    var screen = getActiveScreen(player);
    if (screen instanceof TitleWrapperScreen wrapper) {
      screen = wrapper.delegate();
    }
    if (screen instanceof AbstractWrapperPromptScreen wrapper) {
      screen = wrapper.wrapped;
    }
    return screen instanceof ChatPromptScreen chatScreen ? chatScreen : null;
  }
  ...
  /**
   * Cancels the active screen, timeout, and session for the player, optionally notifying players
   * whose active prompt was cancelled.
   */
  ...
  /**
   * Cancels the active screen, timeout, and session for the player with the specified cancellation
   * mode.
   */
  ...
  /**
   * Cancels the active screen, timeout, and session for the player with the specified cancellation
   * mode and notification flag.
   */
  public void cancelAll(
      Player player, dev.cyr1en.promptpaper.engine.CancellationMode mode, boolean notifyCancelled) {
    ...
    plugin
        .getPluginLogger()
        .debug("Cancelled all for " + player.getName() + " (mode=" + mode + ")");
  }
  ...
  /**
   * Performs bulk teardown of all active custom screen sessions during host CommandPrompter
   * shutdown. Detaches all registrations, cancels sessions, detaches adapter delegates, discards
   * state, and attempts direct platform inventory closure without calling any custom provider
   * delegate methods.
   */
  public void bulkTeardownCustomScreens() {
    if (engine != null
        && engine.getScreenKeyResolver() != null
              ...
              player
                  ...
                  .run(
                      ...
                      scheduledTask ->
                          cancelAll(
                              player,
                              dev.cyr1en.promptpaper.engine.CancellationMode.DISCARD_ONLY,
                              false),
                      () -> discardState(uuid));
          if (task == null) discardState(uuid);
        } catch (Throwable t) {
   ...
   *
   * <p>Enforces cancel-before-close ordering, detaches adapter delegate references, directly closes
   * player platform UI, and never invokes provider delegate methods.
   *
  ...
  private void teardown(
    ...
    teardown(
        ...
        closeScreen,
        notifyCancelled,
        dev.cyr1en.promptpaper.engine.CancellationMode.USER_ACTIONS);
  }
    ...
    try {
      ...
      if (closeScreen && screen != null) {
        try {
          ...
          plugin
              .getPluginLogger()
              .debug("Unable to close screen for " + uuid + ": " + e.getMessage());
        }
      }
    } finally {
      teardownInProgress.remove(uuid);
  ...
  private PromptEngine.DispatchContext takeDispatchContext(UUID uuid) {
    ...
    if (mode == DispatchMode.ATTACHMENT) {
      ...
      return new PromptEngine.DispatchContext(
          dev.cyr1en.promptpaper.preset.ExecuteAs.PLAYER, key, true, permissionSnapshot);
    }
  ...
  private List<String> capturePermissionSnapshot(String permissionKey) {
    ...
    try {
      ...
      plugin
          .getPluginLogger()
          .debug(
              "Unable to capture permission attachment key "
                  + permissionKey
                  + ": "
                  + e.getMessage());
      return List.of();
  ...
  /**
   * Schedules a timeout that auto-cancels the session if the player does not respond within the
   * configured duration.
   */
                  ...
                  scheduledTask -> {
                    ...
                    if (session.isPresent() && session.get().isActive()) {
                      ...
                      if (plugin.getConfigLoader().getConfig().showCancelled()) {
                        player.sendMessage(
                            plugin.getConfigLoader().getI18n().get("prompt.timed_out", player));
                      }
                    }
                  },
