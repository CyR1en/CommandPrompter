...
public record ConfirmationGuiConfig(
  ...
  public static ConfirmationGuiConfig createDefault(Component title, Component infoMessage) {
    return builder().title(title).infoMessage(infoMessage).build();
  }
  ...
  public static class Builder {
    ...
    public ConfirmationGuiConfig build() {
      ...
      var finalFiller =
          fillerItem != null
              ? fillerItem
              : buildItem(
                  Material.GRAY_STAINED_GLASS_PANE, ComponentUtil.mini("<!italic> "), List.of());
    ...
    private static ItemStack buildItem(
        Material material, Component displayName, List<Component> lore) {
