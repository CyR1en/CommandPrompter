 ...
 *
 * <p>Presents a native modal confirmation dialog with heading/body, confirm action, and decline
 * action. All operations run on the player's entity scheduler. Emits {@link
 * ConfirmationOutcome.Confirmed} or {@link ConfirmationOutcome.Declined} exactly once, and emits
 * nothing on programmatic close.
 */
public class ConfirmationDialogView implements ConfirmationView {
  ...
  public ConfirmationDialogView(Plugin plugin, Player player, String title, String body) {
    this(
        plugin,
        player,
  ...
  public ConfirmationDialogView(
      Plugin plugin, Player player, DialogConfig dialogConfig, Component body) {
    this(
        plugin,
        player,
  ...
  public ConfirmationDialogView(
      Plugin plugin, Player player, DialogConfig dialogConfig, String body) {
    this(plugin, player, dialogConfig, body != null ? ComponentUtil.mini(body) : null);
  }
                  ...
                  if (open && !programmaticClose) {
                    handleOpenFailure(
                        new IllegalStateException(
                            "Entity scheduler retired before dialog could be opened"));
                  }
                }
              });
    } catch (Throwable t) {
  ...
  private void closeClientDialog() {
    try {
      player.getScheduler().run(plugin, task -> closeClientDialogDirect(), null);
    } catch (Throwable ignored) {
    }
  }
  ...
  private class NativeDialogLauncher implements DialogLauncher {
    ...
    public void show(Player targetPlayer, Runnable onConfirm, Runnable onDecline) {
      var options = ClickCallback.Options.builder().uses(1).lifetime(Duration.ofMinutes(5)).build();

      var confirmBtnBuilder =
      ...
      var declineBtnBuilder =
          ActionButton.builder(declineLabel)
