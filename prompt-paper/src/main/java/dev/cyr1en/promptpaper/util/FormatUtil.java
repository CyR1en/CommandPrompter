...
/**
 * Utilities for safely handling {@link String#format} specifiers. Validates, counts, escapes, and
 * applies format strings while protecting against malformed specifiers that would throw at runtime.
 */
public final class FormatUtil {
  ...
  /**
   * Returns whether every {@code %}-sequence in the string is a valid {@link String#format}
   * specifier, ignoring ANSI escape sequences.
   */
  public static boolean validFormats(String str) {
    ...
    while (ansiMatcher.find()) {
      if (!validFormatsSegment(str.substring(lastEnd, ansiMatcher.start()))) return false;
  ...
  private static boolean validFormatsSegment(String segment) {
    ...
    while (matcher.find()) {
      if (!FORMAT_SPECIFIER_PATTERN.matcher(matcher.group()).matches()) return false;
    }
    return true;
  }
  ...
  /**
   * Escapes invalid {@code %}-sequences by doubling the leading percent sign so they survive a
   * subsequent {@code String.format} call.
   */
  ...
  /**
   * Applies {@code String.format} safely: escapes invalid specifiers first, pads or truncates the
   * argument array to match the specifier count, and returns the original string unchanged on any
   * formatting error.
   */
  public static String safeFormat(String str, Object... args) {
    if (str == null) return null;
    if (!validFormats(str)) str = escapeInvalidFormats(str);
