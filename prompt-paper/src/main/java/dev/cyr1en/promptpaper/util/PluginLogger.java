...
/**
 * Configurable logger that emits MiniMessage-formatted output through Paper's {@link
 * ComponentLogger}. Supports a {@code fancy} mode with color-gradient prefixes and level-based
 * default colors, plus a {@code debug} mode that automatically prepends the calling class name via
 * {@link StackWalker}.
 *
 * <p>All public methods take a {@link String} (with optional {@link String#format}-style arguments)
 * so callers can pass raw MiniMessage markup like {@code <green>...</green>} or {@code
 * <gradient:#c6a0f6:#8aadf4>...</gradient>} directly. The string is run through {@link
 * FormatUtil#safeFormat(String, Object...)} and then deserialized via {@link
 * MiniMessage#miniMessage()} before being joined to the active prefix and dispatched to the
 * underlying {@link ComponentLogger}.
 */
public class PluginLogger {
    ...
    var normal =
        isFancy
            ? MINI_MESSAGE.deserialize(
                "<gradient:" + NORMAL_GRADIENT + ">" + plainPrefix + "</gradient>")
            : Component.text(plainPrefix);
    var debug =
        isFancy
            ? MINI_MESSAGE.deserialize(
                "<gradient:" + DEBUG_GRADIENT + ">" + debugName + "</gradient>")
  ...
  /**
   * Logs a debug message only when debug mode is enabled. Automatically prepends the calling class
  ...
  public void debug(String msg, Object... args) {
    ...
    var callerName =
        caller != null && !caller.getSimpleName().isBlank() ? caller.getSimpleName() : "?";
  ...
  /**
   * Formats {@code msg} via {@link FormatUtil#safeFormat(String, Object...)}, deserializes the
   * result with {@link MiniMessage#miniMessage()}, appends it to {@code pre}, and dispatches the
   * combined component to the underlying {@link ComponentLogger} at the given {@link Level}.
   */
