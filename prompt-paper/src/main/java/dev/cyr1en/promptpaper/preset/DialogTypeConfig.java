...
/**
 * Represents the {@code dialog_type} block of a {@link DialogPrompt}. Maps to the {@code
 * dialogTypeConfig} definition in the JSON schema.
 *
 * <p>The structure mirrors Paper's native {@code DialogType} hierarchy:
 *
 * <ul>
 *   <li>{@link DialogType#MULTI_ACTION multi_action}: an action grid, optionally populated from
 *       tab-completion. Exactly one of {@link #actions()} or {@link #actionsSource()} must be
 *       provided (enforced by {@link PromptDefinitionDeserializer}).
 ...
 * @param actions explicit action buttons for a multi-action grid (mutually exclusive with {@code
 *     actionsSource})
