...
/**
 * {@code /commandprompter version} — sends the running plugin version to the command sender and
 * logs a debug breadcrumb.
 */
public class VersionCommand extends PromptCommand implements Command<CommandSourceStack> {
  ...
  public VersionCommand(CommandPrompter plugin) {
    super(plugin, "version", "promptpaper.version", null, "Show plugin version", List.of());
  }
  ...
  /**
   * Sends the running plugin version to the given sender. Extracted from the Brigadier executor so
   * it can be unit-tested with a mock {@link CommandSender}.
   */
  public void sendVersion(CommandSender sender) {
    ...
    sender.sendMessage(
        plugin
            .getConfigLoader()
            .getI18n()
            .get("command.version", player, Placeholder.of("version", version)));
