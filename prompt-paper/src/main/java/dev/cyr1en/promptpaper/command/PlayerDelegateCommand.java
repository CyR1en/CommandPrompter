...
/**
 * {@code /playerdelegate <target> <permissionKey> <command>} — starts a prompted command session as
 * the target player with a temporary permission attachment resolved from {@code permissionKey} in
 * the plugin config. The {@code %target_player%} placeholder is resolved on the target's region
 * thread.
 *
 * <p>Restricted to console senders with {@code promptpaper.playerdelegate}. The {@code
 * permissionKey} argument is tab-completed from the configured permission keys.
 */
public class PlayerDelegateCommand extends PromptCommand implements Command<CommandSourceStack> {
  ...
  public PlayerDelegateCommand(CommandPrompter plugin) {
    super(
        ...
        "Execute commands as player with permissions",
        List.of("pd"));
  }
    ...
    return Commands.literal(name())
        ...
        .then(
            argument("target", ArgumentTypes.player())
                .then(
                    argument("permissionKey", StringArgumentType.word())
                        .suggests(
                            (ctx, builder) -> {
                        ...
                        .then(
                            argument("command", StringArgumentType.greedyString()).executes(this))))
        .build();
  }
  ...
  /**
   * Validates the permission key, normalizes the command, and starts the delegated session.
   * Extracted from the Brigadier executor for direct unit testing. Returns {@link
   * Command#SINGLE_SUCCESS} on success or on the unknown-permission-key error path.
   */
  public int executeDispatch(CommandSender sender, Player target, String permKey, String command) {
    if (command.startsWith("/")) command = command.substring(1);
    var config = plugin.getConfigLoader().getConfig();
    var perms =
        config == null || permKey == null || permKey.isBlank()
            ? null
    ...
    plugin.getPluginLogger().info(sender.getName() + " used /playerdelegate permKey=" + permKey);
    plugin
        .getScreenManager()
        .startDelegatedSession(target, command, ScreenManager.DispatchMode.ATTACHMENT, permKey);
    return Command.SINGLE_SUCCESS;
  }
}
