...
/**
 * {@code /commandprompter reload} — cancels every active session across every online player,
 * reloads the configuration from disk, and rebuilds the command-line parser (so {@code
 * Argument-Regex} and {@code Ignore-MiniMessage} take effect without a restart). The cancel pass
 * prevents stale sessions from holding references to the previous config values.
 */
public class ReloadCommand extends PromptCommand implements Command<CommandSourceStack> {
  ...
  public ReloadCommand(CommandPrompter plugin) {
    super(plugin, "reload", "promptpaper.reload", null, "Reload configuration", List.of());
  }
  ...
  /**
   * Cancels all active sessions and reloads the configuration from disk. Extracted from the
   * Brigadier executor so it can be unit-tested directly with a mock {@link CommandSender}.
   */
  public void executeReload(CommandSender sender) {
    ...
    try {
      ...
      try {
        sendResult(sender, feedbackLocation, "command.reload.failed", reloadError(t.getMessage()));
      } finally {
    ...
    for (var player : players) {
      ...
      try {
        ...
        plugin
            .getPluginLogger()
            .debug("Player teardown scheduling failed for " + uuid + ": " + t.getMessage());
        discardPlayerState(uuid, finish);
      }
    }
  }
  ...
  private void scheduleReload(
    ...
    try {
      plugin.getScheduler().runSync(() -> reloadAfterTeardown(sender, feedbackLocation, gateOwner));
    } catch (Throwable t) {
   ...
   * i18n context (so PaperI18n/PapiExpander can expand {@code %...%} for that player); console and
   * block senders use context-free formatting. The message key and placeholders are passed through
   * so no pre-localized {@link Component} ever loses the sender context.
   */
  void sendResult(
      CommandSender sender, Location feedbackLocation, String key, Placeholder... placeholders) {
    ...
    if (sender instanceof BlockCommandSender blockSender) {
      if (feedbackLocation == null) {
        plugin
            .getPluginLogger()
            .debug("Unable to route reload result: block sender has no captured location");
        return;
      }
      try {
        Bukkit.getRegionScheduler()
            .run(plugin, feedbackLocation, scheduledTask -> blockSender.sendMessage(message));
      } catch (Exception e) {
        plugin
            .getPluginLogger()
            .debug("Unable to route reload result to block sender: " + e.getMessage());
      }
      return;
    }
  ...
  private Location captureBlockLocation(CommandSender sender) {
    if (!(sender instanceof BlockCommandSender blockSender)) return null;
    try {
  ...
  private void discardPlayerState(UUID uuid, Runnable finish) {
    try {
      ...
      plugin
          .getPluginLogger()
          .debug("Unable to discard player state for " + uuid + ": " + t.getMessage());
    } finally {
      finish.run();
    }
  }
