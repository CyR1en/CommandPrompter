...
public interface PlayerExecutor {
    ...
    /**
     * Creates a production {@link PlayerExecutor} backed by Paper's Folia-safe entity scheduler
     * and regional ownership checks.
     *
     * <p>If the current thread already owns the target player's region context according to Paper's
     * {@link Bukkit#isOwnedByCurrentRegion(org.bukkit.entity.Entity)}, the task is executed
     * inline synchronously to avoid TOCTOU races. Otherwise, the task is queued on the player's
     * entity scheduler with retirement handling.
     *
    ...
    /**
     * Creates a {@link PlayerExecutor} with a custom ownership predicate, primarily for testing
     * or custom regional routing.
     *
     * @param plugin the plugin owning the task
     * @param player the target player
     * @param ownershipPredicate predicate returning true if the current thread owns the player context
     * @return a PlayerExecutor instance
     */
    static PlayerExecutor forPlayer(Plugin plugin, Player player, Predicate<Player> ownershipPredicate) {
        return (task, retired) -> {
            if (player == null) {
                if (retired != null) {
                    retired.run();
            ...
            try {
                var scheduledTask = player.getScheduler().run(
                        plugin,
                        st -> task.run(),
