 ...
 * safety, authoritative session verification, exactly-once delivery, and provider lifecycle containment.
 */
public class CustomScreenAdapter implements InputScreen {
    ...
    private final AtomicReference<Consumer<Throwable>> openFailureCallbackRef = new AtomicReference<>();
    ...
    /**
     * Constructs a new {@link CustomScreenAdapter} with an existing delegate instance without a retirement callback.
     */
    public CustomScreenAdapter(
            CustomScreenHandle handle,
            InputScreen delegate,
    ...
    private CustomScreenAdapter(
        ...
        this.delegateSupplierRef = new AtomicReference<>(Objects.requireNonNull(delegateSupplier, "delegateSupplier"));
        this.playerExecutor = Objects.requireNonNull(playerExecutor, "playerExecutor");
        this.snapshot = Objects.requireNonNull(snapshot, "snapshot");
        this.verifier = Objects.requireNonNull(verifier, "verifier");
        this.retirementCallbackRef.set(retirementCallback);
    ...
    /**
     * Creates a {@link CustomScreenAdapter} with a lazy delegate supplier invoked on the player's thread.
     *
     * @param handle the registration handle / provider token
     * @param delegateSupplier supplier that creates the third-party input screen delegate on the player thread
     * @param playerExecutor executor for scheduling work on the target player's region/thread
     * @param snapshot immutable session contract expected for this attempt
     * @param verifier post-hop callback reading authoritative live session state
     * @param retirementCallback optional callback invoked when the scheduler is retired
    ...
    public static CustomScreenAdapter lazy(
        ...
        return new CustomScreenAdapter(handle, delegateSupplier, playerExecutor, snapshot, verifier, retirementCallback, true);
    }

    /**
     * Creates a {@link CustomScreenAdapter} with a lazy delegate supplier without a retirement callback.
     */
    public static CustomScreenAdapter lazy(
            CustomScreenHandle handle,
            Supplier<InputScreen> delegateSupplier,
    ...
    public void open() {
        ...
        playerExecutor.execute(() -> {
            ...
            if (delegate == null) {
                ...
                if (supplier != null) {
                    try {
                        ...
                        if (delegate == null) {
                            if (terminalDelivered.compareAndSet(false, true)) {
                                deliverError(new IllegalStateException("Custom screen factory returned null or provider inactive"));
                            }
                            return;
                        }
            ...
            boolean opened = handle.runIfActive(() -> {
                try {
                    finalDelegate.open();
                } catch (Throwable t) {
                    if (terminalDelivered.compareAndSet(false, true)) {
    ...
    private void bindDelegateCallbacks() {
        ...
        if (delegate != null && delegateCallbacksRegistered.compareAndSet(false, true)) {
            boolean bound = handle.runIfActive(() -> {
                try {
    ...
    private void handleDelegateResult(ScreenResult result) {
        // Advisory pre-hop check
        if (detached.get() || !handle.isActive()) {
            return;
        }

        playerExecutor.execute(() -> {
            // 1. Verify provider still ACTIVE
            if (detached.get() || !handle.isActive()) {
                return;
            }

            // 2. Invoke authoritative live verifier
            if (!verifier.verify(snapshot)) {
                return;
            }

            // 3. Check-and-set exactly-once
            if (!terminalDelivered.compareAndSet(false, true)) {
                return;
            }

            // 4. Deliver ScreenResult
    ...
    private void handleDelegateOpenFailure(Throwable error) {
        ...
        playerExecutor.execute(() -> {
            ...
            // 4. Deliver error
            deliverError(error != null ? error : new IllegalStateException("Open failure with null error"));
        }, this::handleRetirement);
    }
