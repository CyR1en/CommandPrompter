 ...
 * zero provider delegate calls after teardown, best-effort platform UI closure, and audit logging.</p>
 *
 * <h2>Platform UI Closure &amp; Provider Limitations</h2>
 * <p>CommandPrompter performs a best-effort {@link Player#closeInventory()} attempt for online players
 * whose entity scheduler accepts work. It does NOT guarantee closure of Paper dialogs, signs, protocol UIs,
 * arbitrary non-inventory UI, offline players, or retired schedulers. Providers own non-inventory/critical
 * external cleanup and must not depend on {@link InputScreen#close()} during disable, as third-party delegate
 * methods are strictly NOT called once provider teardown has commenced.</p>
 */
public class ProviderLifecycleCoordinator {
    ...
    /**
     * Constructs a {@link ProviderLifecycleCoordinator} backed by Paper's entity scheduler and
     * host plugin audit logger if available.
     *
     * @param registry the custom screen registry
    ...
    public ProviderLifecycleCoordinator(
        ...
        this(
                registry,
                screenManager,
                (hostPlugin instanceof dev.cyr1en.promptpaper.CommandPrompter cp && cp.getCustomScreenAuditLogger() != null)
    ...
    /**
     * Constructs a {@link ProviderLifecycleCoordinator} with an explicit audit logger backed by Paper's entity scheduler.
     *
     * @param registry the custom screen registry
     * @param screenManager the screen manager
     * @param hostPlugin the CommandPrompter plugin instance
     ...
     *
     * <p>Idempotent: if the plugin has no active registrations, this is a safe no-op.
     * When active registrations exist:
     * <ol>
     *   <li>Atomically unregisters and detaches all screen factories for the provider.</li>
     *   <li>Snapshots cached provider IDs, keys, owner names, and all active player UUIDs owned by the provider.</li>
     *   <li>Schedules teardown tasks on each active player's entity scheduler.</li>
     *   <li>On player scheduler: re-checks active handle ownership, cancels session with {@link CancelReason#MANUAL},
     *       unlinks state, calls {@link CustomScreenAdapter#teardownDetach()}, and makes a best-effort
     *       {@link Player#closeInventory()} attempt for online players whose entity scheduler accepts work.
     *       It does NOT guarantee closure of Paper dialogs, signs, protocol UIs, arbitrary non-inventory UI,
     *       offline players, or retired schedulers. Providers own non-inventory/critical external cleanup and
     *       must not depend on {@link InputScreen#close()} during disable.</li>
     *   <li>If player scheduler is retired, performs map-only discard without invoking provider or player APIs.</li>
     * </ol>
     *
    ...
    public void onProviderDisable(Plugin ownerPlugin) {
        ...
        // 1. Atomically unregister and detach all registrations for this plugin before player scheduling
        ...
        auditLogger.onAuditEvent(new CustomScreenAuditEvent(
                CustomScreenAuditEvent.Type.TEARDOWN,
                "",
                safeOwnerName,
                -1L,
                "Teardown started: " + unregistered.size() + " screen(s), " + affectedPlayerUuids.size() + " active player(s)"
        ));

        // 2. Schedule teardown for each affected player on their Folia-safe entity scheduler
        for (UUID playerUuid : affectedPlayerUuids) {
            ...
            try {
                ...
                executor.execute(() -> {
                    ...
                    // Retired scheduler path: map-only discard and zero provider calls
                    screenManager.discardState(playerUuid);
                });
            } catch (Throwable t) {
                screenManager.discardState(playerUuid);
    ...
    private void teardownPlayerSession(Player player, Set<Long> providerIds) {
        ...
        if (providerId == null || !providerIds.contains(providerId)) {
            // Session is either no longer active or belongs to another provider / built-in screen
            return;
        }
