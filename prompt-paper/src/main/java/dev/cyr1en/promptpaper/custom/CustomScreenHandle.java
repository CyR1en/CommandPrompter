 ...
 *
 * <p>Does not leak mutable registry internal state or direct Plugin references to adapter
 * consumers.
 */
public interface CustomScreenHandle {
   ...
   *
   * @deprecated Do not invoke the factory outside of the lifecycle gate. Use {@link #invokeFactory}
   *     instead.
   */
  @Deprecated
  ...
  /**
   * Executes the given action under the provider's lifecycle gate if and only if the provider is
   * currently in the {@link ProviderState#ACTIVE} state.
   *
  ...
  /**
   * Computes a value from the given supplier under the provider's lifecycle gate if and only if the
   * provider is currently in the {@link ProviderState#ACTIVE} state.
   *
  ...
  /**
   * Invokes the screen factory under the provider's lifecycle gate if and only if the provider is
   * currently in the {@link ProviderState#ACTIVE} state and has a non-null factory.
   *
