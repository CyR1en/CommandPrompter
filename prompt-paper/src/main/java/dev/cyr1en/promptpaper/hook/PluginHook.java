...
/**
 * Base interface for all external plugin integrations. Hooks are discovered by {@link
 * HookContainer} via {@link dev.cyr1en.promptpaper.hook.annotations.TargetPlugin} and instantiated
 * only when the target plugin is present on the server.
 */
