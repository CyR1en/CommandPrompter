...
/**
 * Hook for the Towny plugin. Registers two player-list filters: {@code tt} (players in the same
 * town) and {@code tn} (players in the same nation).
 */
...
public class TownyHook extends BaseHook implements FilterHook {
  ...
  private static class TownFilter extends CacheFilter {
    TownFilter() {
      super(Pattern.compile("tt"), "TownyTown");
    }
  ...
  private static class NationFilter extends CacheFilter {
    NationFilter() {
      super(Pattern.compile("tn"), "TownyNation");
    }
