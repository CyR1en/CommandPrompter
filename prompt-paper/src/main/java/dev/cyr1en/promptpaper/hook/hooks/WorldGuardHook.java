...
/**
 * Hook for the WorldGuard plugin. Registers three player-list filters: {@code wgr} (players in the
 * same region), {@code wgrm<id>;} (region members), and {@code wgro<id>;} (region owners).
 */
@TargetPlugin(pluginName = "WorldGuard")
public class WorldGuardHook extends BaseHook implements FilterHook {

  public WorldGuardHook(CommandPrompter plugin) {
    super(plugin);
  }
  ...
  /**
   * Extracts online players whose UUIDs appear in a WorldGuard region's domain (members or owners),
   * using the provided domain-extraction function.
   */
  private static List<Player> filterDomain(
      Player relative, String regionId, Function<ProtectedRegion, DefaultDomain> extract) {
  ...
  private static class RegionFilter extends CacheFilter {
    RegionFilter() {
      super(Pattern.compile("wgr"), "WorldGuardRegion");
    }
