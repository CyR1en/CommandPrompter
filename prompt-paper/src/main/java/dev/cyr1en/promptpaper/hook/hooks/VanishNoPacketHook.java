...
/**
 * Hook for the VanishNoPacket plugin. Checks invisibility via {@link VanishPlugin#getManager()} and
 * also respects the {@code vanish.hooks.dynmap.alwayshidden} and join-vanished permissions.
 */
...
public class VanishNoPacketHook extends VanishHook implements Listener {
  ...
  public VanishNoPacketHook(CommandPrompter plugin) {
    ...
    if (jPlugin instanceof VanishPlugin vp) this.vanishPlugin = vp;
    else
      plugin.getLogger().warning("VanishNoPacketHook cannot be initialized without VanishNoPacket");
  }

  /**
   * Checks invisibility via VanishNoPacket's manager, including always-hidden and join-vanished
   * permissions.
   */
  @Override
  public boolean isInvisible(Player player) {
    ...
    if (player.hasPermission("vanish.hooks.dynmap.alwayshidden")
        || VanishPerms.joinVanished(player)) return true;
