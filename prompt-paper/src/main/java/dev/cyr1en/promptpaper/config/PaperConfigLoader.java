...
public class PaperConfigLoader {
  ...
  public PreparedReload prepareReload() {
    ...
    plugin
        .getLogger()
        .fine(
            "config.yml loaded: timeout="
                + newConfig.promptTimeout()
                + " debug="
                + newConfig.debugMode()
                + " fancy="
                + newConfig.fancyLogger()
                + " locale="
                + newConfig.locale());
    if (newConfig.isUsingDeprecatedArgumentRegex()) {
      plugin
          .getLogger()
          .warning(
              "Argument-Regex is deprecated; using derived delimiters '"
                  + newConfig.parserConfig().opening()
                  + "' and '"
                  + newConfig.parserConfig().closing()
                  + "'. Please migrate to Syntax.Prompt.Open and Syntax.Prompt.Close.");
    } else if (newConfig.hasArgumentRegexDisagreement()) {
      plugin
          .getLogger()
          .warning(
              "Argument-Regex is deprecated and conflicts with Syntax.Prompt; using Syntax.Prompt.");
    }
    var newPromptConfig = configManager.getConfig(PromptConfig.class);
    plugin
        .getLogger()
        .fine("prompt-config.yml loaded: mappings=" + newPromptConfig.getScreenMappings().size());
    ...
    var customRegistry =
        plugin != null
            ? (plugin.getCustomScreenRegistry() != null
                ...
                : (plugin.getScreenKeyResolver() != null
                    ? plugin.getScreenKeyResolver().customRegistry()
                    : null))
            : null;
    if (customRegistry != null) {
      customRegistry.validateMappingsAndPublish(
          prepared.promptConfig().getScreenMappings(),
          () -> {
