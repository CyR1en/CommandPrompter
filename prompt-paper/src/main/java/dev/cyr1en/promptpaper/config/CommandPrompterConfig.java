...
@SectionComments({
    @SectionComment(path = "Syntax", comments = {
        "Syntax Configuration"
    }),
    @SectionComment(path = "Syntax.Prompt", comments = {
        "Prompt tag delimiter configuration"
    }),
    @SectionComment(path = "Syntax.Template", comments = {
        "Template placeholder syntax configuration"
    }),
...
public record CommandPrompterConfig(
        ...
        @NodeDefault("256")
        @IntegerConstraint(min = 1, max = dev.cyr1en.promptcore.session.PromptSession.MAX_ANSWER_LENGTH)
        ...
        @NodeDefault("<.*?>")
        @NodeComment({"DEPRECATED: Use Syntax.Prompt instead.",
    ...
    public CommandPrompterConfig {
        ...
        if (maxAnswerLength < 1 || maxAnswerLength > dev.cyr1en.promptcore.session.PromptSession.MAX_ANSWER_LENGTH) {
            throw new IllegalArgumentException(
     ...
     * at its defaults, prompt delimiters are derived from {@code Argument-Regex} for backward compatibility.
     * Otherwise, {@code Syntax.Prompt} is authoritative.
     */
    ...
    public boolean hasArgumentRegexDisagreement() {
        ...
        if (!("<".equals(syntaxPromptOpen) && ">".equals(syntaxPromptClose))) {
            try {
                var derived = ParserConfig.fromArgumentRegex(argumentRegex);
                return !derived.opening().equals(syntaxPromptOpen) || !derived.closing().equals(syntaxPromptClose);
