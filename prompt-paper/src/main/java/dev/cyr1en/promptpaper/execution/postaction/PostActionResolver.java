...
/**
 * Pure resolver translating raw {@link PostCommandMeta} definitions into fully prepared {@link
 * ResolvedPostAction} instances ready for execution.
 *
 * <p>Enforces:
 *
 * <ul>
 *   <li>Authoritative execution policy filtering (presets override parser hints; inline uses
 *       onCancel marker).
 *   <li>Captured snapshot resolution without live singleton lookup.
 *   <li>Fail-closed error semantics on unknown preset IDs, compile errors, or condition evaluation
 *       failures.
 *   <li>Untrusted inline execution templates without PAPI parsing.
 *   <li>Strict condition evaluation with per-reference PAPI validation (max 1024, C0-safe, missing
 *       fail closed).
 * </ul>
 */
public final class PostActionResolver {
  ...
  /**
   * Resolves a single {@link PostCommandMeta} into a {@link ResolvedPostAction} using default
   * template syntax.
   *
   ...
   * @throws PostActionResolutionException if resolution fails closed (unknown preset, compile
  ...
  /**
   * Resolves a single {@link PostCommandMeta} into a {@link ResolvedPostAction} using the specified
   * template syntax.
   *
   * @param pcm the post-command metadata
   * @param lifecyclePolicy active lifecycle policy (ON_COMPLETE or ON_CANCEL)
  ...
  private static ResolvedPostAction resolveLegacyInline(
      PostCommandMeta pcm, ExecutionPolicy lifecyclePolicy, TemplateSyntax syntax) {
    ...
    try {
      template =
          ActionTemplateCompiler.compile(rawCommand, syntax, ActionTrustLevel.UNTRUSTED_INLINE);
    ...
    ExecuteAs executeAs =
        (pcm.dispatchTarget() == DispatchTarget.CONSOLE) ? ExecuteAs.CONSOLE : ExecuteAs.PLAYER;

    return ResolvedPostAction.of(
        template, executeAs, pcm.delayTicks(), ActionProvenance.untrustedInline(), null);
  }
  ...
  private static ResolvedPostAction resolvePreset(
    ...
    Optional<ConditionalPostCommandDefinition> conditionalPostCommand =
        snapshot.getConditionalPostCommand(presetId);
    ...
    if (legacyPostCommand.isPresent()) {
      ...
      try {
        template =
            ActionTemplateCompiler.compile(def.command(), syntax, ActionTrustLevel.TRUSTED_PRESET);
      } catch (ActionTemplateException e) {
        throw new PostActionResolutionException(
            DispatchErrorKind.INVALID_REQUEST,
          ...
          public Optional<String> getPlaceholder(String placeholder) {
            ...
            try {
              ...
              throw new ConditionEvaluationException(
                  "PlaceholderAPI resolution failed for %" + placeholder + "%: " + t.getMessage(),
                  t);
            }
    ...
    try {
      template =
          ActionTemplateCompiler.compile(
              branch.command().source(), syntax, ActionTrustLevel.TRUSTED_PRESET);
    } catch (ActionTemplateException e) {
      throw new PostActionResolutionException(
          DispatchErrorKind.INVALID_REQUEST,
          "Failed to compile conditional branch template for preset '"
              + presetId
              + "': "
              + e.getMessage(),
          e);
