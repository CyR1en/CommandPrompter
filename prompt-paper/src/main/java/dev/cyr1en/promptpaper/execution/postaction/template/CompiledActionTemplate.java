 ...
 *
 * <p>Evaluates in a single pass during {@link #render(ActionTemplateBindings, MathMode, int)}. All
 * substituted values (player name, inputs, PAPI expansions) are treated as opaque literal data and
 * are never reparsed or re-expanded.
 */
public record CompiledActionTemplate(
    String source, ActionTrustLevel trustLevel, List<ActionTemplateSegment> segments) {
  ...
  /**
   * Renders this compiled template using the provided bindings, default math mode, and max output
   * length.
   *
  ...
  /**
   * Renders this compiled template using the provided bindings and math mode with max output
   * length.
   *
   * @param bindings the runtime bindings
  ...
  /**
   * Renders this compiled template using the provided bindings, math mode, and custom output length
   * cap.
   *
   * @param bindings the runtime bindings
   * @param mathMode the math mode to apply to numeric transforms
   ...
   * @return the rendered result
   */
  public ActionTemplateResult render(
      ActionTemplateBindings bindings, MathMode mathMode, int maxOutputLength) {
    ...
    for (ActionTemplateSegment segment : segments) {
      if (segment instanceof ActionTemplateSegment.Literal literal) {
        ...
        if (boundValue != null) {
          ...
          return ActionTemplateResult.failure(
              ActionTemplateErrorCode.MISSING_BINDING, "Missing binding for reference key: " + key);
        }

        SingleTransformResult transformResult =
            coreRef.transformer().transform(boundValue, mathMode);
        if (transformResult.isFailure()) {
          return ActionTemplateResult.failure(
              ActionTemplateError.fromCore(transformResult.error()));
        }
