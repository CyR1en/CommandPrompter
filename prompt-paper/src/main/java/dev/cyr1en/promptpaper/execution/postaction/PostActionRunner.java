...
/**
 * Sequential runner for post-actions (immediate, delayed, conditional) attached to an
 * {@link ExecutionPlanInstance}.
 *
 * <p>Enforces:
 * <ul>
 *   <li>Sequential execution in exact source order; next action begins only after prior delay + dispatch completion.
 *   <li>Strict caller and instance verification using immutable tokens captured at run start.
 *   <li>Enforces object identity between provided InputCompletion and instance-owned InputCompletion.
 *   <li>Owned cancellable handles registered with {@link ExecutionPlanInstance} and tracked for cleanup.
 *   <li>Thread-safe re-entry onto initiator {@link PlayerExecutor} from arbitrary dispatcher callback threads.
 *   <li>Synchronous dispatch/scheduler exception wrapping and exactly-once completion callback invocation.
 *   <li>Transform DIV0 notice recording through {@link NoticeFlag} without core logger dependencies.
 * </ul>
 */
public final class PostActionRunner {
    ...
    private final ExecutionStage capturedStage;
    ...
    private final AtomicInteger stepTokenSequence;
    ...
    private static final class StepExecution {
        ...
        StepExecution(int stepToken, int index) {
    ...
    private PostActionRunner(
        ...
        this.initiatorExecutor = Objects.requireNonNull(initiatorExecutor, "initiatorExecutor must not be null");
        this.instance = Objects.requireNonNull(instance, "instance must not be null");
        this.lifecyclePolicy = Objects.requireNonNull(lifecyclePolicy, "lifecyclePolicy must not be null");
        ...
        this.completionMismatched = (inputCompletion != null && inputCompletion != this.inputCompletion);

        this.capturedExecutionId = instance.getExecutionId();
        this.capturedInitiatorUuid = player.getUniqueId();
        this.capturedIncarnation = this.inputCompletion != null ? this.inputCompletion.incarnation() : -1L;
        this.capturedStage = ExecutionStage.POST_ACTIONS;

        this.postCommands = this.inputCompletion != null ? this.inputCompletion.postCommands() : List.of();
        this.completed = new AtomicBoolean(false);
        this.activeDelayCancellable = new AtomicReference<>(null);
        this.stepTokenSequence = new AtomicInteger(0);
        this.currentStep = new AtomicReference<>(null);
    }

    /**
     * Executes the post-actions associated with the given plan instance and completion state
    ...
    /**
     * Executes the post-actions associated with the given plan instance, completion state, and template syntax.
     * Enforces that {@code inputCompletion} must match {@code instance.getInputCompletion()}.
     *
    ...
    public static void execute(
        ...
        PostActionRunner runner = new PostActionRunner(
                player,
                initiatorExecutor,
                instance,
                inputCompletion,
    ...
    private void start() {
        if (completionMismatched) {
            ...
            notifyComplete(PostActionResult.failure(
                    DispatchErrorKind.INVALID_REQUEST,
                    "InputCompletion does not match instance-owned completion for execution " + capturedExecutionId
            ));
            return;
        }

        if (this.inputCompletion == null) {
            instance.recordNotice(NoticeFlag.POST_ACTION_FAILED);
            notifyComplete(PostActionResult.failure(
                    DispatchErrorKind.INVALID_REQUEST,
                    "Instance InputCompletion is null for execution " + capturedExecutionId
            ));
            return;
        }

        if (!instance.verifyCaller(capturedInitiatorUuid, capturedIncarnation)
                || !instance.verify(capturedInitiatorUuid, capturedIncarnation, capturedExecutionId)) {
            instance.recordNotice(NoticeFlag.STALE_CALLER_DETECTED);
            notifyComplete(PostActionResult.failure(
                    DispatchErrorKind.INVALID_REQUEST,
        ...
        if (instance.getStage() != capturedStage) {
            notifyComplete(PostActionResult.failure(
                    DispatchErrorKind.INVALID_REQUEST,
    ...
    private void executeStep(int index) {
        ...
        int token = stepTokenSequence.incrementAndGet();
        StepExecution step = new StepExecution(token, index);
        currentStep.set(step);

        PostCommandMeta pcm = postCommands.get(index);
        ResolvedPostAction resolved;
        try {
            resolved = PostActionResolver.resolve(pcm, lifecyclePolicy, inputCompletion, papiResolver, syntax);
        ...
        try {
            ActionTemplateBindings bindings = ActionTemplateBindings.of(
                    inputCompletion.answers(),
                    player.getName(),
                    papiResolver
            );
        ...
        if (!renderResult.notices().isEmpty()) {
            for (TransformNotice notice : renderResult.notices()) {
                if (notice == TransformNotice.DIVISION_BY_ZERO_SUBSTITUTED) {
        ...
        if (delayTicks < 0 || delayTicks > MAX_DELAY_TICKS) {
            ...
            notifyComplete(PostActionResult.failure(
                    DispatchErrorKind.INVALID_REQUEST,
                    "Delay ticks out of bounds: " + delayTicks
            ));
            return;
        }
    ...
    private void scheduleDelayedAction(
        ...
        if (completed.get() || instance.isTerminal() || step.timerCancelled.get() || currentStep.get() != step) {
        ...
        Runnable timerTask = () -> {
            try {
                ...
                if (step.timerCancelled.get() || completed.get() || instance.isTerminal() || currentStep.get() != step) {
                    return;
                }
                if (!instance.verifyCaller(capturedInitiatorUuid, capturedIncarnation)
                        || !instance.verify(capturedInitiatorUuid, capturedIncarnation, capturedExecutionId)) {
        ...
        try {
            CancellableTask rawTask = scheduler.scheduleDelayed(player, timerTask, retiredCallback, delayTicks);
    ...
    private void dispatchAction(StepExecution step, String command, ResolvedPostAction resolved) {
        ...
        ImmediateActionRequest request = new ImmediateActionRequest(
                player,
                command,
                resolved.executeAs(),
                resolved.provenance(),
        ...
        try {
            dispatcher.dispatch(request, outcome -> {
                ...
                try {
                    // Dispatcher callbacks may be from arbitrary thread; always re-enter initiator PlayerExecutor
                            ...
                            () -> {
                                try {
                                    ...
                                    if (outcome != null && outcome.isSuccess()) {
                                        ...
                                        instance.recordNotice(NoticeFlag.POST_ACTION_FAILED);
                                        DispatchError error = (outcome != null && outcome.optionalError().isPresent())
                                                ? outcome.optionalError().get()
                                                : DispatchError.of(
