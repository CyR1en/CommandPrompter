...
/**
 * Guards PlaceholderAPI calls so the {@link PapiDelegate} class is only referenced (and therefore
 * loaded) when the {@code PlaceholderAPI} plugin is enabled at runtime.
 *
 * <p>This indirection prevents a {@link NoClassDefFoundError} on servers that do not have
...
final class PapiExpander {
  ...
  /**
   * Expands PAPI placeholders in {@code text} when PAPI is available and {@code player} is
   * non-null; otherwise returns {@code text} unchanged.
   *
