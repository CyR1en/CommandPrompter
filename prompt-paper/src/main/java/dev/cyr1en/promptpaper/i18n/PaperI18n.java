 ...
 *
 * <p>Produces {@link Component} instances from translated message strings using the following
 * formatting pipeline:
 *
 * <ol>
 *   <li>{@link #preFormat} — expands PlaceholderAPI {@code %papi_placeholders%} via {@link
 *       PapiExpander} (safe: no-op when PAPI is not installed)
 *   <li>{@link AbstractI18n#applyPlaceholders} — replaces {@code %key%} tokens with the supplied
 *       {@link Placeholder} values
 *   <li>{@link #postFormat} — parses the resulting string with MiniMessage to produce the final
 *       {@link Component}
 * </ol>
 *
 * <p>Call sites use the inherited {@link #get(String, Placeholder...)} and {@link #get(String,
 * Player, Placeholder...)} overloads.
 */
public class PaperI18n extends AbstractI18n<Component, Player> {
   ...
   * @param baseDir the plugin data folder (used to locate the user-override {@code locales/}
   *     directory)
   ...
   *
   * <p>This runs <em>before</em> the custom {@code %key%} substitution so that PAPI results can
   * themselves contain {@code %key%} tokens if needed. The call is guarded by {@link PapiExpander}
   * — it is safe even when PAPI is absent.
   *
