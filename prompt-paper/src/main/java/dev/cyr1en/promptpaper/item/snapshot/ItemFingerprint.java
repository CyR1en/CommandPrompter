...
/**
 * Immutable fingerprint representing an item's real material, real amount, normalized SHA-256
 * metadata hash, and serialized byte length.
 *
 * @param material The item material (never null)
 * @param amount The real item amount (>= 0)
 * @param sha256 The SHA-256 hex string of the canonical serialization of the item normalized to
 *     amount 1 (never null)
 * @param serializedByteCount The serialized payload size in bytes (>= 0)
 */
public record ItemFingerprint(
    Material material, int amount, String sha256, int serializedByteCount) {
  ...
  public ItemFingerprint {
    ...
    if (serializedByteCount < 0) {
      throw new IllegalArgumentException(
          "serializedByteCount cannot be negative: " + serializedByteCount);
    }
  }
  ...
  /**
   * Checks if this fingerprint exactly matches another fingerprint in material, amount, and
   * normalized SHA-256 hash. Empty fingerprints are considered matching any other empty
   * fingerprint.
   */
  ...
  /**
   * Checks if this fingerprint matches another fingerprint in material and normalized SHA-256 hash,
   * ignoring amount.
   */
  public boolean matchesIgnoringAmount(ItemFingerprint other) {
    ...
    return this.material == other.material && this.sha256.equalsIgnoreCase(other.sha256);
  }
}
