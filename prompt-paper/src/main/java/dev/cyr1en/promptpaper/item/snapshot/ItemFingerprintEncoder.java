 ...
 *
 * <ul>
 *   <li>Normalization of a defensive clone to amount 1 prior to canonical Paper serialization
 *   <li>Per-item size bound of 64 KiB (65,536 bytes)
 *   <li>Aggregate screen size bound of 2 MiB (2,097,152 bytes)
 *   <li>Fail-closed error handling on serialization failures or size limit violations
 * </ul>
 */
public final class ItemFingerprintEncoder {
  ...
  public static ItemFingerprint encode(ItemStack item) {
    ...
    try {
      ...
      throw new ItemSerializationException(
          "Failed to canonically serialize item " + material + ": " + ex.getMessage(), ex);
    }

    if (serializedBytes == null) {
      throw new ItemSerializationException(
          "Canonical item serialization returned null for " + material);
    }

    if (serializedBytes.length > MAX_ITEM_BYTES) {
      throw new OversizedItemException(
          "Serialized item size exceeds 64 KiB limit ("
  ...
  /**
   * Encodes a map of slot-to-item entries into slot-to-fingerprint entries and validates the 2 MiB
   * screen aggregate.
   *
  ...
  /**
   * Captures and encodes all 41 player inventory slots (0..40) and validates the 2 MiB screen
   * aggregate.
   *
