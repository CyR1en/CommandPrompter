 ...
 *
 * @param material the resolved Bukkit {@link Material} (guaranteed to satisfy {@link
 *     Material#isItem()})
 * @param key the canonical {@link NamespacedKey} for the material
 * @param canonicalKey the string form of the namespaced key (e.g., {@code
...
public record CatalogEntry(Material material, NamespacedKey key, String canonicalKey) {
   ...
   *
   * @param rawKey the raw string (e.g., {@code "diamond_sword"}, {@code "minecraft:stone"}, {@code
   *     "IRON_INGOT"})
   * @return the resolved {@link CatalogEntry}
   * @throws ItemCatalogException if the string cannot be resolved or does not represent a valid
   *     item
   */
