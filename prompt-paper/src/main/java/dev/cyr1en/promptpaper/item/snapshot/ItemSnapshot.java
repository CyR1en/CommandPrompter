...
/**
 * Immutable snapshot of an item at a specific inventory slot, captured at a point in time.
 * Enforces defensive copying of the contained {@link ItemStack}.
 */
public final class ItemSnapshot {
    ...
    public ItemSnapshot(int playerSlot, OptionalInt guiSlot, ItemStack item, ItemFingerprint fingerprint, long timestampMillis) {
    ...
    public static ItemSnapshot of(int playerSlot, OptionalInt guiSlot, ItemStack item, ItemFingerprint fingerprint) {
        return new ItemSnapshot(playerSlot, guiSlot, item, fingerprint, System.currentTimeMillis());
    }

    public static ItemSnapshot empty(int playerSlot) {
        OptionalInt guiSlot = ItemSlotMapping.toGuiSlot(playerSlot);
        return new ItemSnapshot(playerSlot, guiSlot, ItemStack.empty(), ItemFingerprint.EMPTY, System.currentTimeMillis());
    }
    ...
    public static ItemSnapshot captureGuiSlot(PlayerInventory inventory, int guiSlot) {
        ...
        ItemFingerprint fingerprint = ItemFingerprintEncoder.encode(item);
        return new ItemSnapshot(playerSlot.getAsInt(), OptionalInt.of(guiSlot), item, fingerprint, System.currentTimeMillis());
    }

    /**
     * Captures snapshots of all 41 slots in the player inventory and validates the 2 MiB screen aggregate limit.
     */
