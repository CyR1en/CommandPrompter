 ...
 *
 * <p>Serves as the single source of truth for catalog lookups at runtime. All readers obtain an
 * immutable {@link CatalogSnapshot} via {@link #getSnapshot()} or {@link #snapshot()}, ensuring
 * thread-safety and consistent state across in-flight GUI screens.
 *
 * <h2>Failure semantics</h2>
 *
 * <p>I/O, YAML syntax, category constraint, or material validation failures throw {@link
 * ItemCatalogException}. The previous snapshot is left untouched so that a broken edit on disk does
 * not disrupt live servers.
 */
public class ItemCatalogRegistry {
  ...
  /**
   * Constructs a registry for testing or non-standard paths with a custom default resource
   * supplier.
   *
   * @param catalogFile the on-disk file
   * @param defaultResource supplier for the bundled default resource (may return null to skip
   *     extraction)
   */
   ...
   *
   * <ol>
   *   <li>Ensures the catalog file exists, extracting the default resource if missing.
   *   <li>Parses and validates the YAML document off-side against all limits and invariants.
   *   <li>Atomically swaps in the newly constructed {@link CatalogSnapshot}.
   * </ol>
   *
   * @throws ItemCatalogException if the file cannot be read, exceeds 1 MiB, is malformed, violates
   *     category limits/patterns, lacks required {@code "all"}, contains invalid materials, or
  ...
  private void ensureFileExists() {
    ...
    if (defaultResource != null) {
      try (InputStream in = defaultResource.get()) {
        ...
        throw new ItemCatalogException(
            "Failed to copy default catalog resource to '" + catalogFile + "': " + e.getMessage(),
            e);
      }
    } else {
