...
/**
 * Result of an item selection verification check. Encapsulates success/failure state, defensive
 * copy of the verified {@link ItemStack}, failure reason, diagnostic error message, and optional
 * exception cause.
 */
public final class VerificationResult {
  ...
  /**
   * Creates a failed verification result with reason, diagnostic message, and actual item
   * fingerprint observed.
   */
  public static VerificationResult failure(
      VerificationFailureReason reason, String errorMessage, ItemFingerprint actualFingerprint) {
    Objects.requireNonNull(reason, "failureReason cannot be null on failure");
    return new VerificationResult(false, null, actualFingerprint, reason, errorMessage, null);
  }

  /**
   * Creates a failed verification result with reason, diagnostic message, and underlying throwable
   * cause.
   */
  public static VerificationResult failure(
      VerificationFailureReason reason, String errorMessage, Throwable cause) {
    Objects.requireNonNull(reason, "failureReason cannot be null on failure");
  ...
  /**
   * Returns a defensive copy of the verified item, or throws {@link IllegalStateException} if
   * verification failed.
   */
  public ItemStack getItemOrThrow() {
    if (!success || item == null) {
      throw new IllegalStateException(
          "Cannot retrieve item from failed verification result: " + errorMessage);
    }
    return item.clone();
  }
