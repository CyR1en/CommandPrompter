...
/**
 * Pure and fail-closed verifier for player item selections against captured {@link ItemSnapshot} or {@link ItemFingerprint} data.
 * <p>
 * Designed to be safely invoked from an entity-scheduler-owned screen.
 */
public final class ItemSelectionVerifier {
    ...
    /**
     * Verifies the item currently in a player's inventory at the logical slot mapped from the provided GUI slot.
     *
     ...
     */
    public static VerificationResult verifyGuiSlot(PlayerInventory inventory, int guiSlot, ItemSnapshot expectedSnapshot) {
        if (inventory == null) {
            return VerificationResult.failure(VerificationFailureReason.NULL_INVENTORY, "Inventory cannot be null");
        }
        if (expectedSnapshot == null) {
            return VerificationResult.failure(VerificationFailureReason.NULL_SNAPSHOT, "Expected snapshot cannot be null");
        }
        ...
        if (expectedSnapshot.playerSlot() != playerSlot) {
            return VerificationResult.failure(
                    ...
                    "GUI slot " + guiSlot + " maps to player slot " + playerSlot
                            + " but expected snapshot is for slot " + expectedSnapshot.playerSlot()
            );
        }
     ...
     * @return {@link VerificationResult} describing success or explicit failure reason
     */
    public static VerificationResult verifyPlayerSlot(PlayerInventory inventory, int playerSlot, ItemSnapshot expectedSnapshot) {
        if (inventory == null) {
            return VerificationResult.failure(VerificationFailureReason.NULL_INVENTORY, "Inventory cannot be null");
        }
        if (expectedSnapshot == null) {
        ...
        if (!ItemSlotMapping.isValidPlayerSlot(playerSlot)) {
            return VerificationResult.failure(
                    VerificationFailureReason.SLOT_OUT_OF_BOUNDS,
                    "Player slot index " + playerSlot + " is out of bounds (0.." + (ItemSlotMapping.TOTAL_PLAYER_SLOTS - 1) + ")"
            );
        }
        if (expectedSnapshot.playerSlot() != playerSlot) {
            return VerificationResult.failure(
                    VerificationFailureReason.SLOT_OUT_OF_BOUNDS,
                    "Target player slot " + playerSlot + " does not match expected snapshot slot " + expectedSnapshot.playerSlot()
            );
        }
        ...
        try {
            ...
            return VerificationResult.failure(
                    ...
                    "Failed to retrieve item from player inventory slot " + playerSlot + ": " + ex.getMessage(),
                    ex
            );
        }
     ...
     * @return {@link VerificationResult} describing success or explicit failure reason
     */
    public static VerificationResult verifyItem(ItemStack currentItem, ItemFingerprint expectedFingerprint) {
        if (expectedFingerprint == null) {
            return VerificationResult.failure(VerificationFailureReason.NULL_SNAPSHOT, "Expected fingerprint cannot be null");
        }
        ...
        if (currentEmpty) {
            return VerificationResult.failure(
                    ...
                    "Slot is empty but expected item " + expectedFingerprint.material() + " x" + expectedFingerprint.amount(),
        ...
        if (expectedEmpty) {
            ...
            return VerificationResult.failure(
                    ...
                    "Slot contains item " + safeCurrent.getType() + " x" + safeCurrent.getAmount() + " but expected empty slot",
                    actualFp
            );
        }
        ...
        if (safeCurrent.getType() != expectedFingerprint.material()) {
            ...
            return VerificationResult.failure(
                    ...
                    "Item material changed: expected " + expectedFingerprint.material() + " but found " + safeCurrent.getType(),
                    actualFp
            );
        }
        ...
        if (safeCurrent.getAmount() != expectedFingerprint.amount()) {
            ...
            return VerificationResult.failure(
                    ...
                    "Item amount changed: expected " + expectedFingerprint.amount() + " but found " + safeCurrent.getAmount(),
                    actualFp
            );
        }
        ...
        try {
            ...
            return VerificationResult.failure(
                    VerificationFailureReason.ITEM_OVERSIZED,
                    ex.getMessage(),
                    ex
            );
        ...
        if (!actualFp.sha256().equalsIgnoreCase(expectedFingerprint.sha256())) {
            return VerificationResult.failure(
                    ...
                    "Item metadata hash mismatch: expected " + expectedFingerprint.sha256() + " but found " + actualFp.sha256(),
                    actualFp
            );
        }
    ...
    /**
     * Verifies the entire inventory against a set of expected snapshots and ensures the 2 MiB aggregate limit is respected.
     *
     * @param inventory         The player inventory (must not be null)
     ...
     */
    public static VerificationResult verifyInventory(PlayerInventory inventory, Map<Integer, ItemSnapshot> expectedSnapshots) {
        if (inventory == null) {
            return VerificationResult.failure(VerificationFailureReason.NULL_INVENTORY, "Inventory cannot be null");
        }
        if (expectedSnapshots == null) {
            return VerificationResult.failure(VerificationFailureReason.NULL_SNAPSHOT, "Expected snapshots map cannot be null");
        }
        ...
        for (Map.Entry<Integer, ItemSnapshot> entry : expectedSnapshots.entrySet()) {
            ...
            if (result.getFingerprint().isPresent()) {
                try {
                    ...
                    return VerificationResult.failure(
                            VerificationFailureReason.AGGREGATE_OVERSIZED,
                            ex.getMessage(),
                            ex
                    );
                }
