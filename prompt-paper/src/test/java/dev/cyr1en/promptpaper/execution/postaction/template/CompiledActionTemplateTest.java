...
class CompiledActionTemplateTest {
  ...
  void testRenderLegacyPlaceholders() {
    var template =
        ActionTemplateCompiler.compile("tell {player} {input} {input:1} {input:2} {0} {1}");
  ...
  void testTransformers() {
    var template =
        ActionTemplateCompiler.compile(
            "echo {player:upper} {0:lower} {1:capitalize} {2:trim} {3:stripcolor}");
    var bindings =
        ActionTemplateBindings.of(
            List.of("UPPER_TO_LOWER", "steve", "   spaced   ", "&aColored"), "alex");
  ...
  void testDefaultTransformer() {
    var template =
        ActionTemplateCompiler.compile(
            "give {player} {0:default=\"diamond\"} {1:default=\"gold\"}");
  ...
  void testTrustedPapiRendering() {
    var template =
        ActionTemplateCompiler.compile(
            "say %player_name% balance: %vault_eco_balance%", ActionTrustLevel.TRUSTED_PRESET);
    var resolver =
        PapiReferenceResolver.fromMap(
            Map.of(
                "player_name", "Alex",
  ...
  void testMissingPapiResolverFails() {
    var template =
        ActionTemplateCompiler.compile("say %player_name%", ActionTrustLevel.TRUSTED_PRESET);
  ...
  void testPapiResolutionFailure() {
    var template =
        ActionTemplateCompiler.compile("say %unknown_papi%", ActionTrustLevel.TRUSTED_PRESET);
    ...
    var throwingResolver =
        (PapiReferenceResolver)
            token -> {
  ...
  void testPapiValueTooLong() {
    var template =
        ActionTemplateCompiler.compile("say %long_val%", ActionTrustLevel.TRUSTED_PRESET);
    var resolver =
        PapiReferenceResolver.fromMap(
            Map.of("long_val", "X".repeat(ActionTemplateLimits.MAX_INPUT_LENGTH + 1)));
    var bindings = ActionTemplateBindings.of(List.of(), "Alex", resolver);
  ...
  void testPapiControlCharacter() {
    ...
    var resolver = PapiReferenceResolver.fromMap(Map.of("bad_val", "bad\u0000text"));
    var bindings = ActionTemplateBindings.of(List.of(), "Alex", resolver);
  ...
  void testBoundInputTooLong() {
    ...
    var bindings =
        ActionTemplateBindings.of(
            List.of("X".repeat(ActionTemplateLimits.MAX_INPUT_LENGTH + 1)), "Alex");
