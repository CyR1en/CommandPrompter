...
class ActionTemplateCompilerTest {
    ...
    void testCompileLiteral() {
        ...
        assertEquals("say hello world", ((ActionTemplateSegment.Literal) template.segments().get(0)).text());
    }
    ...
    void testCompilePlaceholdersWithTransformers() {
        var template = ActionTemplateCompiler.compile("msg {player:upper} {0:lower} {input:1:capitalize} {input:math(+5)}");
    ...
    void testEscapes() {
        ...
        assertEquals(1, template.segments().size());
        assertEquals("test {escaped} \\ %literal%", ((ActionTemplateSegment.Literal) template.segments().get(0)).text());
    }
    ...
    void testSourceTooLong() {
        ...
        var ex = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile(longSource));
    ...
    void testControlCharacters() {
        var ex1 = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile("say \u0000 bad"));
        assertEquals(ActionTemplateErrorCode.CONTROL_CHARACTER_DETECTED, ex1.errorCode());

        var ex2 = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile("say \n newline"));
        assertEquals(ActionTemplateErrorCode.CONTROL_CHARACTER_DETECTED, ex2.errorCode());

        var ex3 = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile("say \u007F del"));
    ...
    void testUnclosedPlaceholder() {
        var ex = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile("say {player"));
    ...
    void testUnescapedClosingBrace() {
        var ex = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile("say hello}world"));
        assertEquals(ActionTemplateErrorCode.MALFORMED_TEMPLATE, ex.errorCode());
    }
    ...
    void testEmptyPlaceholderKey() {
        var ex = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile("say {}"));
    ...
    void testZeroIndexedInputPlaceholder() {
        var ex = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile("say {input:0}"));
        assertEquals(ActionTemplateErrorCode.MALFORMED_PLACEHOLDER, ex.errorCode());
    }
    ...
    void testChainedTransformers() {
        var ex = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile("say {player:upper:lower}"));
    ...
    void testUnknownTransformer() {
        var ex = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile("say {player:invalidTransform}"));
    ...
    void testTrustedPapiCompilation() {
        var template = ActionTemplateCompiler.compile("say %player_name% has %vault_eco_balance%", ActionTrustLevel.TRUSTED_PRESET);
        ...
        assertEquals("vault_eco_balance", ((ActionTemplateSegment.Papi) template.segments().get(3)).token());
    ...
    void testUntrustedPapiCompilation() {
        var template = ActionTemplateCompiler.compile("say %player_name% has %vault_eco_balance%", ActionTrustLevel.UNTRUSTED_INLINE);
        assertEquals(ActionTrustLevel.UNTRUSTED_INLINE, template.trustLevel());
        assertEquals(1, template.segments().size());
        assertInstanceOf(ActionTemplateSegment.Literal.class, template.segments().get(0));
        assertEquals("say %player_name% has %vault_eco_balance%", ((ActionTemplateSegment.Literal) template.segments().get(0)).text());
    ...
    void testRawPercentInTrustedMode() {
        var template = ActionTemplateCompiler.compile("give 100% bonus % %invalid token% %% %", ActionTrustLevel.TRUSTED_PRESET);
        assertEquals(1, template.segments().size());
        assertInstanceOf(ActionTemplateSegment.Literal.class, template.segments().get(0));
        assertEquals("give 100% bonus % %invalid token% %% %", ((ActionTemplateSegment.Literal) template.segments().get(0)).text());
        assertTrue(template.papiTokens().isEmpty());
    }
    ...
    void testSegmentLimit() {
        ...
        var ex = assertThrows(ActionTemplateException.class, () -> ActionTemplateCompiler.compile(sb.toString()));
