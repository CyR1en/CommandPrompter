...
class ActionTemplateSecurityTest {
  ...
  @Test
  @DisplayName(
      "SEC-01: Answer containing PAPI placeholder remains literal and resolver is NOT called for it")
  void testAnswerContainingPapiPlaceholder() {
    var template =
        ActionTemplateCompiler.compile(
            "broadcast {player} answered: {0}", ActionTrustLevel.TRUSTED_PRESET);

    AtomicInteger resolverCallCount = new AtomicInteger(0);
    PapiReferenceResolver resolver =
        token -> {
    ...
    var bindings =
        ActionTemplateBindings.of(List.of("%player_name% %vault_eco_balance%"), "Alex", resolver);
  ...
  @Test
  @DisplayName(
      "SEC-01: Answer containing command separators or ops remains literal and is not split or re-evaluated")
  ...
  void testAnswerContainingMiniMessageTags() {
    ...
    var bindings =
        ActionTemplateBindings.of(
            List.of("<c:red><click:run_command:/op attacker>Click me</click></c>"), "Alex");
  ...
  @Test
  @DisplayName(
      "SEC-01: Answer containing template syntax {player} or {0} remains literal and is not re-parsed")
  void testAnswerContainingBraceSyntax() {
    var template = ActionTemplateCompiler.compile("log {player} -> {0}");
    var bindings = ActionTemplateBindings.of(List.of("{player:upper} {1} {input:999}"), "Alex");
  ...
  @Test
  @DisplayName(
      "FLOW-11: Trusted source PAPI is called exactly once per compiled ref and duplicate refs are deterministic")
  void testTrustedSourcePapiResolution() {
    var template =
        ActionTemplateCompiler.compile(
            "stats %score% and %score% for %player_name%", ActionTrustLevel.TRUSTED_PRESET);
  ...
  @Test
  @DisplayName(
      "SEC-01: PAPI expansion containing nested PAPI or command syntax remains opaque data and is never re-expanded")
  void testPapiExpansionContainingNestedPlaceholders() {
    var template =
        ActionTemplateCompiler.compile("eval %nested_data%", ActionTrustLevel.TRUSTED_PRESET);
    ...
    assertEquals(
        1,
        callCount.get(),
        "Only the precompiled token was resolved; nested token was treated as literal data");
  }

  @Test
  @DisplayName(
      "SEC-01: Untrusted inline compile treats all percent tokens as literal text without calling resolver")
  void testUntrustedInlineCompileIsolation() {
    var template =
        ActionTemplateCompiler.compile(
            "say %player_name% %vault_eco_balance%", ActionTrustLevel.UNTRUSTED_INLINE);
