...
class PaperImmediateActionDispatcherTest extends MockBukkitTest {
  ...
  void allowsTrustedPresetForAuthorizedPlayer() {
    ...
    var request =
        ImmediateActionRequest.trustedPreset(player, "/actconsoleok", ExecuteAs.CONSOLE, "preset1");
    dispatcher.dispatch(
        request,
        outcome -> {
          count.incrementAndGet();
  ...
  void allowsTrustedPresetForOp() {
    ...
    var outcomeRef = new AtomicReference<DispatchOutcome>();
    var request =
        ImmediateActionRequest.trustedPreset(player, "actconsoleop", ExecuteAs.CONSOLE, "preset2");
  ...
  void refusesUntrustedInlineActionAsConsole() {
    ...
    player.setOp(true); // even if player is op, untrusted inline cannot elevate to console without
    // delegation
  ...
  void refusesTrustedPresetWhenPlayerLacksPermission() {
    ...
    var outcomeRef = new AtomicReference<DispatchOutcome>();
    var request =
        ImmediateActionRequest.trustedPreset(
            player, "actrefused", ExecuteAs.CONSOLE, "preset_denied");
