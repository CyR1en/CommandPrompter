...
public class MockBukkitTest {
  ...
  void setUp() {
    ...
    when(plugin.getPluginLoader())
        .thenReturn(new org.mockbukkit.mockbukkit.plugin.MockBukkitPluginLoader());
    ...
    when(config.templateSyntax())
        .thenReturn(dev.cyr1en.promptcore.logic.transform.TemplateSyntax.DEFAULT);
    ...
    when(promptConfig.dialogConfig())
        .thenReturn(
            DialogConfig.legacy(
                "Prompt",
                "<green>Confirm</green>",
    ...
    when(i18n.get("validation.invalid_integer"))
        .thenReturn(Component.text("Please enter a valid integer."));
    when(i18n.get("validation.invalid_string"))
        .thenReturn(Component.text("Input cannot be empty."));
    when(i18n.get("validation.answer_too_long"))
        .thenReturn(Component.text("Input exceeds maximum length."));
    when(i18n.get("prompt.error.invalid_title_filter"))
        .thenReturn(
            Component.text(
                "Invalid prompt configuration: TITLE filter cannot be used on a non-compound tag."));
    when(i18n.get("command.error.players_only"))
        .thenReturn(Component.text("Only players can cancel prompts."));
    when(i18n.get("command.cancel.no_active_prompt"))
    ...
    when(i18n.get("player_ui.search_instruction"))
        .thenReturn(Component.text("Type your search term in chat."));
    when(i18n.get("dialog.no_options"))
    ...
    when(i18n.get(eq("prompt.error.command_failed"), any(Player.class), any(Placeholder[].class)))
        .thenReturn(Component.text("Command failed."));
    when(i18n.get(
            eq("command.delegate.unknown_permission"), any(Player.class), any(Placeholder[].class)))
        .thenReturn(Component.text("Unknown permission key."));
