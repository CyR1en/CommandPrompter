 ...
 *
 * <p>Verifies that overlaying preset definitions does not discard any metadata on
 * {@link PromptTag} (such as custom flags and compiled breakIf conditions) or
 * {@link ParsedCommand} (such as pre-dispatch gate specifications and template spans).
 */
class PromptEnginePresetSanitizeRegressionTest extends MockBukkitTest {
    ...
    void applyPresetSanitizePreservesAllTagAndCommandMetadata() {
        var breakCondition = ConditionCompiler.compile("{0} == \"cancel\"", ConditionCompileOptions.forPreset());
        var titleConfig = new TitleConfig("Title", "Subtitle", 60);
        var customFlags = Map.of("rarity", "epic", "theme", "dark");
        var originalTag = new PromptTag(
                "<@test_prompt>",
                "",
                null,
                "test_prompt",
        ...
        var parsedBase = new CommandLineParser(ParserConfig.ANGLE_BRACKETS).parse("/trade <@test_prompt> <!log> <!gate:@trade_gate>");
        ...
        assertFalse(adjustedTag.sanitize(), "Authoritative preset sanitize=false must override parsed default");
    ...
    void interceptPreservesMetadataEndToEnd() {
        ...
        var interceptResult = engine.intercept(player, "/approve <@confirm_preset> <!gate:@admin_approval>");
        ...
        assertFalse(sessionTag.sanitize(), "Preset definition sanitize=false must take effect in session tag");
        assertEquals(1, session.parsedCommand().preDispatchGates().size(), "PreDispatchGates must be preserved in session");
        assertEquals("admin_approval", ((PreDispatchGateSpec.Approval) session.parsedCommand().preDispatchGates().get(0)).presetId());
    }

    @Test
    @DisplayName("applyPresetSanitize returns original ParsedCommand unchanged when no preset tags exist")
