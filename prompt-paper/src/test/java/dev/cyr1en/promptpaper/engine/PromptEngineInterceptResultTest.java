...
class PromptEngineInterceptResultTest extends MockBukkitTest {
    ...
    void setUp() {
        customRegistry = new CustomScreenRegistry(
                () -> true,
                Map::of,
                Set.of("a", "anvil", "s", "sign", "p", "player", "d", "dialog", "c", "confirm", "confirmation"),
                CustomScreenAuditLogger.noop()
        );
        resolver = new ScreenKeyResolver(
                customRegistry,
                Map.of("mapped_anvil", dev.cyr1en.promptpaper.config.ScreenType.ANVIL)
        );
    ...
    void testRegisteredCustomKeyStartsSession() {
        ...
        assertEquals("my_custom", ((InterceptResult.Started) result).parsedCommand().promptTags().get(0).key());
    }
    ...
    void testUnknownScreenKeyRejectsFailClosed() {
        ...
        verify(plugin.getConfigLoader().getI18n(), atLeastOnce()).get(eq("command.error.missing_preset"), same(player));
    }
    ...
    void testBuiltInsAndConfiguredMappings() {
        ...
        assertInstanceOf(InterceptResult.Started.class, engine.interceptResult(player, "/cmd <a:Anvil>"));
        engine.discard(player.getUniqueId());

        assertInstanceOf(InterceptResult.Started.class, engine.interceptResult(player, "/cmd <s:Sign>"));
        engine.discard(player.getUniqueId());

        assertInstanceOf(InterceptResult.Started.class, engine.interceptResult(player, "/cmd <p:Player>"));
        engine.discard(player.getUniqueId());

        assertInstanceOf(InterceptResult.Started.class, engine.interceptResult(player, "/cmd <d:Dialog>"));
        engine.discard(player.getUniqueId());

        assertInstanceOf(InterceptResult.Started.class, engine.interceptResult(player, "/cmd <c:Confirm>"));
        engine.discard(player.getUniqueId());

        assertInstanceOf(InterceptResult.Started.class, engine.interceptResult(player, "/cmd <:Chat>"));
        engine.discard(player.getUniqueId());

        assertInstanceOf(InterceptResult.Started.class, engine.interceptResult(player, "/cmd <mapped_anvil:Mapped>"));
        engine.discard(player.getUniqueId());
    }
    ...
    void testKnownPresetPrompt() {
        var chat = new ChatPrompt("chat", "my_preset", "Why?", new CancelBehavior(false, "", false, ""), true);
    ...
    void testKnownPresetPostCommandNoPrompts() {
        var pc = new PostCommand("log_preset", "say {player} hi", ExecutionPolicy.ON_COMPLETE, ExecuteAs.CONSOLE, 0);
    ...
    void testDiagnosticsSanitizationAndTruncation() {
        ...
        assertInstanceOf(InterceptResult.RejectedFailClosed.class, result);
        verify(plugin.getPluginLogger()).warn(argThat(msg ->
