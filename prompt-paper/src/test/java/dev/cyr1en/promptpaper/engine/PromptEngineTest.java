...
class PromptEngineTest extends MockBukkitTest {
  ...
  /**
   * Issue #99 integration proof: the reload-gate feedback is sent directly to the player, so it
   * must be formatted with that player as the i18n context. The mocked i18n answers differently
   * depending on whether the player argument is supplied; the recipient must see the player-context
   * result.
   */
  @Test
  ...
  /**
   * Issue #99: an active-session rejection message is sent directly to the player, so it must be
   * formatted with that player as the i18n context.
   */
  @Test
  void activeSessionRejectionUsesPlayerContext() {
    var engine = new PromptEngine(plugin, scheduler);
    var player = createPlayer();
    when(i18n.get("prompt.error.session_active")).thenReturn(Component.text("context-free"));
