...
class ScreenManagerBreakIfTest extends MockBukkitTest {
    ...
    void setUpBreakIfTests() {
        ...
        screenManager = new ScreenManager(plugin, engine, factory, scheduler, (p, tag, ctx) -> {
    ...
    private long countCapturedCommands(String prefix) {
        return capturedCommands.stream()
                .filter(c -> c.command().contains(prefix))
                .count();
    }
    ...
    @Test
    @DisplayName("FLOW-06: Single prompt with breakIf=true cancels with MANUAL, dispatches cancel PCM, no primary dispatch")
    void singlePromptBreakIfTrueCancelsWithManualAndDispatchesCancelPCMs() {
        ...
        assertEquals(0, countCapturedCommands("custom_test"), "Primary command must NOT be dispatched on breakIf");
        ...
        assertEquals(1, countCapturedCommands("on_cancel"), "On-cancel PCM must be dispatched on breakIf");
    }
    ...
    @Test
    @DisplayName("FLOW-06: Multi-prompt condition referencing previous and candidate indices evaluates accurately")
    void multiPromptPreviousAndNewIndicesBreakIf() {
        ...
        // Step 2: Prompt 1 answered with "ignore" -> condition {0} equals "admin" || {1} equals "ignore" evaluates with ["steve", "ignore"] -> true
        ...
        var cancelCmd = capturedCommands.stream()
                .filter(c -> c.command().startsWith("on_cancel") || c.command().startsWith("/on_cancel"))
                .findFirst()
                .orElseThrow();
        assertTrue(cancelCmd.command().contains("steve"), "Cancel PCM must resolve accepted answers: " + cancelCmd.command());
        assertFalse(cancelCmd.command().contains("ignore"), "Cancel PCM must NOT contain unsubmitted candidate answer: " + cancelCmd.command());
    }

    @Test
    @DisplayName("FLOW-06: Multi-prompt with previous answers evaluates to false and finishes successfully")
    ...
    @Test
    @DisplayName("FLOW-06: Compound dialog batch evaluated against combined answers before submission")
    ...
    void compoundDialogPromptBatchBreakIfFalse() {
        ...
        screenManager.startSession(
                player,
                "/custom_dlg <d:text:A && d:text:B -breakIf:{0} equals \"x\" && {1} equals \"y\">");
        assertTrue(engine.hasActiveSession(player));
    ...
    void confirmationPromptValueModeBreakIf() {
        ...
        var screen = new ConfirmationPromptScreen(List.of(view), "true", "false", false, null, true); // valueMode = true
        var customManager = new ScreenManager(plugin, engine, factory, scheduler, (p, t, c) -> screen);

        customManager.startSession(player, "/custom_pvp <c:Enable? -value -breakIf:{0} equals \"true\"> <!!on_cancel>");
        assertTrue(engine.hasActiveSession(player));
    ...
    void confirmationPromptValueModeBreakIfFalse() {
        ...
        customManager.startSession(player, "/custom_pvp <c:Enable? -value -breakIf:{0} equals \"false\">");
        assertTrue(engine.hasActiveSession(player));
    ...
    @Test
    @DisplayName("FLOW-06: Unresolvable runtime answer index fails closed with CancelReason.ERROR and bounded diagnostic")
    void unresolvableRuntimeIndexFailsClosedError() {
        ...
        screenManager.startSession(
                player,
                "/custom_test <a:Input -breakIf:{5} equals \"test\"> <!!error_pcm>");
        assertTrue(engine.hasActiveSession(player));
        ...
        assertFalse(screenManager.hasActiveScreen(player));
        assertEquals(0, countCapturedCommands("custom_test"), "Primary command must NOT be dispatched on error");
        assertEquals(1, countCapturedCommands("error_pcm"), "Error cancel PCM must be dispatched");

        // Verify warning diagnostic is logged
        assertTrue(capturedWarnMessages.stream().anyMatch(m ->
                m.contains("BreakIf evaluation failed for UnresolvableUser")
        ), "Warning diagnostic must be logged for breakIf evaluation failure");
    }

    @Test
    @DisplayName("FLOW-06: Type mismatch during runtime evaluation fails closed with CancelReason.ERROR")
    void typeMismatchRuntimeErrorFailsClosedError() {
        ...
        screenManager.startSession(
                player,
                "/custom_test <a:Input -breakIf:{0} <= 10> <!!error_pcm>");
        assertTrue(engine.hasActiveSession(player));
    ...
    @Test
    @DisplayName("FLOW-06: Validation failure does not evaluate breakIf condition and re-shows prompt")
    void validationFailureDoesNotEvaluateBreakIf() {
        ...
        screenManager.startSession(
                player,
                "/custom_test <a:Age -int -breakIf:{0} == 0> <!!on_cancel>");
        ...
        assertTrue(engine.hasActiveSession(player), "Session must remain active when validator rejects answer");
    ...
    @Test
    @DisplayName("FLOW-06: Duplicate callback is discarded by stale check and does not re-evaluate breakIf")
    void duplicateCallbackDoesNotEvaluateTwice() throws Exception {
        var player = createPlayer("DuplicateUser");
        screenManager.startSession(
                player,
                "/custom_test <a:Reason -breakIf:{0} equals \"skip\"> <!!on_cancel>");
        assertTrue(engine.hasActiveSession(player));
