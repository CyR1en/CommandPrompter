...
class ConfirmationFlowIntegrationTest extends MockBukkitTest {
  ...
  void init() {
    ...
    when(pConfig.getScreenMappings())
        .thenReturn(java.util.Map.of("", dev.cyr1en.promptpaper.config.ScreenType.CHAT));
  ...
  void conf03_gatingDeclineNoPrimaryDispatchAndExactlyOneCancelPcm() {
    ...
    assertEquals(
        0, countCapturedCommands("delete"), "Primary command must NOT be dispatched on decline");
    assertEquals(
        1,
        countCapturedCommands("cancel_pcm"),
        "Cancellation PCM must be dispatched exactly once on decline");
    assertFalse(engine.hasActiveSession(player));
    assertFalse(customManager.hasActiveScreen(player));
    assertEquals(0, nonceRegistry.size(), "Nonces must be cleaned up on decline");
    assertEquals(
        0, rateLimiter.attemptCount(player.getUniqueId()), "Rate limiter must be reset on decline");
  }
  ...
  void conf03_gatingDeclineThroughResponseServiceNoPrimaryDispatchAndOneCancelPcm() {
    ...
    screenManager.startSession(
        player, "/delete world <c:Delete world? -mode:chat> <!!cancel_pcm decline>");
    assertTrue(engine.hasActiveSession(player));
    assertTrue(screenManager.hasActiveScreen(player));
  ...
  void conf04_scheduledTimeoutCancelsWithTimeoutReasonAndDispatchesCancelPcm() {
    ...
    screenManager.startSession(
        player, "/delete world <c:Are you sure? -timeout:1> <!!timeout_pcm timeout>");
    assertTrue(engine.hasActiveSession(player));
    assertTrue(screenManager.hasActiveScreen(player));
    ...
    assertEquals(
        0, countCapturedCommands("delete"), "Primary command must NOT dispatch on timeout");
    assertEquals(
        1,
        countCapturedCommands("timeout_pcm"),
        "Timeout cancellation PCM must dispatch exactly once");
    ...
    assertEquals(
        0,
        rateLimiter.attemptCount(player.getUniqueId()),
        "Rate limiter must be reset after timeout");
  }
  ...
  void conf05_expiredNonceThroughResponseCommandPathNoStateChange() {
    ...
    assertTrue(
        ...
        "Session state must NOT change on expired nonce submission");
  ...
  void conf06_07_callbackBeforeScheduledTimeoutRaceExecutesSingleTerminalOutcome() {
    ...
    customManager.startSession(
        player, "/delete world <c:Delete world? -timeout:1> <!!timeout_pcm timeout>");
    assertTrue(engine.hasActiveSession(player));
    ...
    assertEquals(
        1, countCapturedCommands("delete"), "Primary command must be dispatched on confirm");
    assertEquals(
        0, countCapturedCommands("timeout_pcm"), "Timeout PCM must NOT be dispatched on confirm");
    ...
    assertEquals(
        0,
        countCapturedCommands("timeout_pcm"),
        "Timeout PCM must NOT dispatch after session completed");
    assertFalse(customManager.hasActiveScreen(player));
  ...
  void conf06_07_scheduledTimeoutBeforeCallbackRaceDiscardsStaleOutcome() {
    ...
    assertEquals(
        0,
        countCapturedCommands("delete"),
        "Primary command must NOT dispatch from stale callbacks");
  ...
  void conf06_07_programmaticCloseVsCapturedCallbackGuaranteesNoStaleDeclineOrDoublePcm() {
    ...
    assertEquals(
        1, countCapturedCommands("cancel_pcm"), "Cancel PCM dispatched on programmatic close");
    assertFalse(engine.hasActiveSession(player));
    ...
    assertEquals(
        1,
        countCapturedCommands("cancel_pcm"),
        "Stale callbacks must NOT cause duplicate cancel PCM");
  }
  ...
  void conf08_wrongPlayerWithActiveSessionFailsWithUuidMismatchAndSafeLogTruncation() {
    ...
    assertTrue(
        nonceRegistry.contains(fullNonceA), "Player A's nonce must NOT be consumed by Player B");
    ...
    assertEquals(
        sessionA.generation(),
        engine.getSession(playerA).orElseThrow().generation(),
        "Player A session unchanged");
    assertEquals(
        sessionB.generation(),
        engine.getSession(playerB).orElseThrow().generation(),
        "Player B session unchanged");

    // Verify logs contain PLAYER_MISMATCH, truncated prefix, and NEVER the full nonce
    boolean hasPlayerMismatchLog =
        capturedLogMessages.stream().anyMatch(m -> m.contains("PLAYER_MISMATCH"));
    boolean hasTruncatedPrefix =
        capturedLogMessages.stream().anyMatch(m -> m.contains(truncatedPrefix));
  ...
  void conf09_successfulCompletionCleansUpAllResourcesAndDispatchesExactlyOnce() {
    ...
    customManager.startSession(
        player, "/delete world <c:Delete world?> <!audit success> <!!cancel_pcm decline>");
    var session = engine.getSession(player).orElseThrow();
    ...
    assertEquals(
        1,
        rateLimiter.attemptCount(player.getUniqueId()),
        "Rate limiter state present before confirm");
    ...
    // Assert dispatch
    assertEquals(
        1, countCapturedCommands("delete"), "Primary command must be dispatched exactly once");
    assertEquals(
        1,
        countCapturedCommands("audit"),
        "Success completion PCM must be dispatched exactly once");
    ...
    assertEquals(
        0,
        rateLimiter.attemptCount(player.getUniqueId()),
        "Rate limiter must be reset after completion");
  }
  ...
  void conf10_burstRateLimitingThroughResponseCommandSingleAggregatedLog() {
    ...
    assertEquals(
        1,
        rateLimitWarnings,
        "Rate limiting must emit exactly one counted/rate-bounded warning per window");
  }
  ...
  void nonTerminalAdvancementInvalidatesPreviousPromptNonceWithoutResettingRateLimiter() {
    ...
    assertFalse(
        nonceRegistry.contains(binding0.nonce()),
        "Prompt 0 nonce must be invalidated on advancement");
    assertTrue(
        nonceRegistry.contains(binding1.nonce()),
        "Prompt 1 nonce must remain valid for active prompt");
  }
  ...
  void openFailureAsyncCallbackTriggersErrorTeardownWithCancelPcm() {
    ...
    assertFalse(
        engine.hasActiveSession(player),
        "Session must be cancelled with ERROR on async open failure");
    assertFalse(customManager.hasActiveScreen(player), "Screen must be cleaned up");
    assertEquals(
        0,
        countCapturedCommands("delete"),
        "Primary command must NOT dispatch on async open error");
    assertEquals(
        1, countCapturedCommands("error_pcm"), "Error cancellation PCM must dispatch exactly once");
  ...
  void openFailureSynchronousExceptionTriggersErrorTeardownWithCancelPcm() {
    ...
    var customManager =
        new ScreenManager(plugin, engine, factory, scheduler, (p, t, c) -> throwingScreen);
    ...
    assertFalse(
        engine.hasActiveSession(player), "Session must be cancelled with ERROR on sync open error");
    assertFalse(customManager.hasActiveScreen(player), "Screen must be cleaned up");
    assertEquals(
        0, countCapturedCommands("delete"), "Primary command must NOT dispatch on sync open error");
    assertEquals(
        1, countCapturedCommands("error_pcm"), "Error cancellation PCM must dispatch exactly once");
    assertEquals(0, nonceRegistry.size(), "Nonces must be cleaned up");
    assertEquals(0, rateLimiter.attemptCount(player.getUniqueId()), "Rate limiter must be reset");
  ...
  void disableExecutesRealLifecycleTeardown() {
    ...
    assertEquals(
        0, rateLimiter.attemptCount(player1.getUniqueId()), "onDisable must clear rate limiter");
    assertEquals(
        0, rateLimiter.attemptCount(player2.getUniqueId()), "onDisable must clear rate limiter");
