...
class PlayerDelegateCommandTest extends MockBukkitTest {
  ...
  void setUp() {
    ...
    when(config.getPermissionKeys()).thenReturn(new String[] {"GAMEMODE"});

    var delegateI18n = mock(PaperI18n.class);
    when(delegateI18n.get(
            eq("command.delegate.unknown_permission"), isNull(), any(Placeholder[].class)))
  ...
  void stripsLeadingSlashAndReplacesTargetPlayer() {
    ...
    var result = cmd.executeDispatch(sender, target, "GAMEMODE", "/msg %target_player% hi");
  ...
  void unknownPermissionKeySendsErrorAndDoesNotStartSession() {
    ...
    verify(screenManager, never()).startDelegatedSession(any(), any(), any(), any());
  }
