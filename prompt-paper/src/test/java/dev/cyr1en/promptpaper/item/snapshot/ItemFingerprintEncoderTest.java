...
class ItemFingerprintEncoderTest extends MockBukkitTest {
    ...
    void testItemMetaMutations() {
        ...
        assertNotEquals(fpPlain.sha256(), fpNamed.sha256(), "Named item must produce different hash from plain");
        ...
        assertNotEquals(fpPlain.sha256(), fpEnchanted.sha256(), "Enchanted item must produce different hash");
        assertNotEquals(fpNamed.sha256(), fpEnchanted.sha256(), "Enchanted item must differ from named item");
    ...
    void testOversizedAggregateExceptionDetails() {
        OversizedAggregateException ex = assertThrows(OversizedAggregateException.class, () -> {
    ...
    void testFingerprintInvariants() {
        ...
        assertThrows(NullPointerException.class, () -> new ItemFingerprint(Material.STONE, 1, null, 10));
        assertThrows(IllegalArgumentException.class, () -> new ItemFingerprint(Material.STONE, -1, "abc", 10));
        assertThrows(IllegalArgumentException.class, () -> new ItemFingerprint(Material.STONE, 1, "abc", -1));
    }
    ...
    void testComputeSha256Hex() {
        ...
        assertEquals(ItemFingerprint.EMPTY_SHA256, ItemFingerprintEncoder.computeSha256Hex(new byte[0]));
    }
}
