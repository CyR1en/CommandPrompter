...
class ItemCatalogParserTest extends MockBukkitTest {
  ...
  @Test
  void oversizedStreamIsRejectedWithoutReadingPastTheLimit() {
    int limit = (int) ItemCatalogParser.MAX_FILE_SIZE_BYTES;
    var input = new java.io.ByteArrayInputStream(new byte[limit + 100]);
    assertThrows(
        ItemCatalogException.class, () -> ItemCatalogParser.parse(input, "oversized stream"));
  ...
  void testMissingAllCategory() {
    String yaml =
        """
                ...
                """;
    ItemCatalogException ex =
        assertThrows(ItemCatalogException.class, () -> ItemCatalogParser.parse(yaml, "no_all.yml"));
  ...
  @ParameterizedTest
  @ValueSource(
      strings = {"all", "building_blocks", "food.drinks", "combat-gear", "group_123.test-abc"})
  ...
  void testInvalidCategoryNames(String category) {
    ...
    ItemCatalogException ex =
        assertThrows(
            ItemCatalogException.class, () -> ItemCatalogParser.parse(yaml, "invalid_name.yml"));
    assertTrue(
        ex.getMessage().contains("Invalid category name") || ex.getMessage().contains("regex"));
  }
  ...
  void testOversizedCategoryName() {
    ...
    ItemCatalogException ex =
        assertThrows(
            ItemCatalogException.class, () -> ItemCatalogParser.parse(yaml, "long_name.yml"));
    assertTrue(
        ex.getMessage().contains("Invalid category name") || ex.getMessage().contains("regex"));
  }
  ...
  void testMaxEntriesPerCategoryLimit() {
    StringBuilder sb = new StringBuilder();
    sb.append("categories:\n  all:\n");
    // Add 1025 entries to "all" (even with duplicate or unique materials, size check happens
    // before/during parse)
  ...
  void testDuplicateEntryRejection() {
    ...
    ItemCatalogException ex =
        assertThrows(
            ItemCatalogException.class, () -> ItemCatalogParser.parse(yaml, "duplicate.yml"));
  ...
  void testInvalidMaterialThrows() {
    String invalidMatYaml =
        """
                ...
                """;
    assertThrows(
        ItemCatalogException.class, () -> ItemCatalogParser.parse(invalidMatYaml, "bad_mat.yml"));
    ...
    ItemCatalogException ex =
        assertThrows(
            ItemCatalogException.class, () -> ItemCatalogParser.parse(nonItemYaml, "water.yml"));
  ...
  void testOrderPreservation() {
    ...
    assertEquals(
        List.of(Material.IRON_SWORD, Material.STONE, Material.GOLDEN_APPLE, Material.BOW),
        snapshot.all().stream().map(CatalogEntry::material).toList());
    assertEquals(
        List.of(Material.COMPASS, Material.CLOCK),
