...
class CatalogEntryTest extends MockBukkitTest {
  ...
  void testNullArguments() {
    assertThrows(
        ...
        () -> new CatalogEntry(null, NamespacedKey.minecraft("stone"), "minecraft:stone"));
    assertThrows(
        NullPointerException.class,
        () -> new CatalogEntry(Material.STONE, null, "minecraft:stone"));
    assertThrows(
        NullPointerException.class,
        () -> new CatalogEntry(Material.STONE, NamespacedKey.minecraft("stone"), null));
  }
  ...
  void testResolveNonItem(String nonItem) {
    ItemCatalogException ex =
        assertThrows(ItemCatalogException.class, () -> CatalogEntry.resolve(nonItem));
    assertTrue(
        ex.getMessage().contains("not a valid item")
            || ex.getMessage().contains("isItem() == false"));
  }

  @ParameterizedTest
  @ValueSource(strings = {"", "   ", "invalid_unknown_item_xyz", "minecraft:does_not_exist_123"})
