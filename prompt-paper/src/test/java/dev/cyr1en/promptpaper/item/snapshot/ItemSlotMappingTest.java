...
class ItemSlotMappingTest {
  ...
  void testMainStorageMapping() {
    for (int guiSlot = 0; guiSlot <= 26; guiSlot++) {
      ...
      assertEquals(
          expectedPlayerSlot,
          playerSlot.getAsInt(),
          "GUI slot " + guiSlot + " must map to " + expectedPlayerSlot);
      ...
      assertEquals(
          Optional.of(SlotCategory.MAIN_STORAGE), ItemSlotMapping.getCategoryForGuiSlot(guiSlot));
      assertEquals(
          Optional.of(SlotCategory.MAIN_STORAGE),
          ItemSlotMapping.getCategoryForPlayerSlot(expectedPlayerSlot));
  ...
  void testHotbarMapping() {
    for (int guiSlot = 27; guiSlot <= 35; guiSlot++) {
      ...
      assertEquals(
          Optional.of(SlotCategory.HOTBAR), ItemSlotMapping.getCategoryForGuiSlot(guiSlot));
      assertEquals(
          Optional.of(SlotCategory.HOTBAR),
          ItemSlotMapping.getCategoryForPlayerSlot(expectedPlayerSlot));
  ...
  @Test
  @DisplayName(
      "GUI 46..49 maps to logical player slots 36..39 (armor: boots, leggings, chestplate, helmet)")
  void testArmorMapping() {
    ...
    for (int pSlot = 36; pSlot <= 39; pSlot++) {
      ...
      assertEquals(
          Optional.of(SlotCategory.ARMOR), ItemSlotMapping.getCategoryForPlayerSlot(pSlot));
    }
  }
