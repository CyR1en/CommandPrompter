...
class CompoundedValidatorTest {
  ...
  void allAndPassWhenAllMatch() {
    var c = new CompoundedValidator("test", "fail", null, passingAnd(), passingAnd());
    assertTrue(c.validate("42"));
  }

  @Test
  void anyAndFailWhenOneFails() {
    var c = new CompoundedValidator("test", "fail", null, passingAnd(), failingAnd());
    assertFalse(c.validate("42"));
  }

  @Test
  void orPassesWhenAnyMatches() {
    var c = new CompoundedValidator("test", "fail", null, failingAnd(), passingOr());
