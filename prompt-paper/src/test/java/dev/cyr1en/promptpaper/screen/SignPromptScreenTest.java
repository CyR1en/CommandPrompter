...
class SignPromptScreenTest extends MockBukkitTest {
  ...
  void constructorStoresValues() {
    ...
    var screen =
        new SignPromptScreen(
            plugin,
            player,
            new dev.cyr1en.promptpaper.preset.SignPrompt(
                "sign", "inline-test", "Enter:", java.util.List.of(), true),
            emptyProviders);
  ...
  void openWithValidProviderDelegates() {
    ...
    when(mockProvider.createSign(any(CommandPrompter.class), any(), any())).thenReturn(mockSign);
  ...
  void handleResultWithAnswerFiresCallback() {
    ...
    var screen =
        new SignPromptScreen(
            ...
            player,
            new dev.cyr1en.promptpaper.preset.SignPrompt(
                "sign", "inline-test", "Enter value", java.util.List.of(), true),
            emptyProviders);
  ...
  void handleResultWithoutSanitizePreservesColorCodes() {
    ...
    var screen =
        new SignPromptScreen(
            ...
            player,
            new dev.cyr1en.promptpaper.preset.SignPrompt(
                "sign", "inline-test", "Enter value", java.util.List.of(), false),
            emptyProviders);
    var resultRef = new AtomicReference<ScreenResult>();
    screen.onResult(resultRef::set);
    screen.open();
  ...
  @Test
  void multiArgKeepsAllFourEditableLabels() {
    var player = createPlayer();
    var provider = mock(ScreenProvider.class);
    var sign = mock(SignInputScreen.class);
    var renderedLines = new AtomicReference<String[]>();
    when(provider.createSign(any(CommandPrompter.class), any(), any()))
        .thenAnswer(
            invocation -> {
              renderedLines.set(invocation.getArgument(2));
              return sign;
            });
    var screen =
        new SignPromptScreen(
            ...
            player,
            new dev.cyr1en.promptpaper.preset.SignPrompt(
                "sign", "inline-test", "name:{br}age:{br}email:{br}city:", List.of(), true),
            List.of(provider));
    var result = new AtomicReference<ScreenResult>();
    screen.onResult(result::set);

    screen.open();
    assertArrayEquals(new String[] {"name:", "age:", "email:", "city:"}, renderedLines.get());
    screen.handleResult(ScreenResult.answer("name:John\nage:30\nemail:john@test.com\ncity:Denver"));
    assertEquals("John 30 john@test.com Denver", result.get().answer());
  }
  ...
  void handleResultWithMultiArgExtractsValues() {
    ...
    var screen =
        new SignPromptScreen(
            ...
            player,
            new dev.cyr1en.promptpaper.preset.SignPrompt(
                "sign", "inline-test", "name:{br}age:{br}email:", java.util.List.of(), true),
            emptyProviders);
    var resultRef = new AtomicReference<ScreenResult>();
    screen.onResult(resultRef::set);
    screen.open();
  ...
  void handleResultNonMultiArgPreservesInput() {
    ...
    var screen =
        new SignPromptScreen(
            ...
            player,
            new dev.cyr1en.promptpaper.preset.SignPrompt(
                "sign", "inline-test", "Enter value{br}Confirm", java.util.List.of(), true),
            emptyProviders);
    var resultRef = new AtomicReference<ScreenResult>();
    screen.onResult(resultRef::set);
    screen.open();
  ...
  void arrangeLinesHandlesAllLocations() {
    ...
    // top location
    assertArrayEquals(
        new String[] {"", "Prompt 1", "", ""}, SignPromptScreen.arrangeLines(parts, 1, "top"));
    assertArrayEquals(
        new String[] {"", "Prompt 1", "Prompt 2", ""},
        SignPromptScreen.arrangeLines(parts, 2, "top"));
    assertArrayEquals(
        new String[] {"", "Prompt 1", "Prompt 2", "Prompt 3"},
        SignPromptScreen.arrangeLines(parts, 3, "top"));

    // top-aggregate location
    assertArrayEquals(
        new String[] {"", "", "", "Prompt 1"},
        SignPromptScreen.arrangeLines(parts, 1, "top-aggregate"));
    assertArrayEquals(
        new String[] {"", "", "Prompt 1", "Prompt 2"},
        SignPromptScreen.arrangeLines(parts, 2, "top-aggregate"));
    assertArrayEquals(
        new String[] {"", "Prompt 1", "Prompt 2", "Prompt 3"},
        SignPromptScreen.arrangeLines(parts, 3, "top-aggregate"));

    // bottom location
    assertArrayEquals(
        new String[] {"Prompt 1", "", "", ""}, SignPromptScreen.arrangeLines(parts, 1, "bottom"));
    assertArrayEquals(
        new String[] {"Prompt 1", "Prompt 2", "", ""},
        SignPromptScreen.arrangeLines(parts, 2, "bottom"));
    assertArrayEquals(
        new String[] {"Prompt 1", "Prompt 2", "Prompt 3", ""},
        SignPromptScreen.arrangeLines(parts, 3, "bottom"));

    // bottom-aggregate location
    assertArrayEquals(
        new String[] {"Prompt 1", "", "", ""},
        SignPromptScreen.arrangeLines(parts, 1, "bottom-aggregate"));
    assertArrayEquals(
        new String[] {"Prompt 1", "Prompt 2", "", ""},
        SignPromptScreen.arrangeLines(parts, 2, "bottom-aggregate"));
    assertArrayEquals(
        new String[] {"Prompt 1", "Prompt 2", "Prompt 3", ""},
        SignPromptScreen.arrangeLines(parts, 3, "bottom-aggregate"));

    // fallback default location
    assertArrayEquals(
        new String[] {"Prompt 1", "", "", ""}, SignPromptScreen.arrangeLines(parts, 1, null));
  }
  ...
  void handleResultTopAggregateExtractsNonPromptLines() {
    ...
    var screen =
        new SignPromptScreen(
            ...
            player,
            new dev.cyr1en.promptpaper.preset.SignPrompt(
                "sign", "inline-test", "Prompt 1{br}Prompt 2", java.util.List.of(), true),
            emptyProviders);
    var resultRef = new AtomicReference<ScreenResult>();
    screen.onResult(resultRef::set);
    screen.open();
  ...
  void handleResultBottomAggregateExtractsNonPromptLines() {
    ...
    var screen =
        new SignPromptScreen(
            ...
            player,
            new dev.cyr1en.promptpaper.preset.SignPrompt(
                "sign", "inline-test", "Prompt 1", java.util.List.of(), true),
            emptyProviders);
    var resultRef = new AtomicReference<ScreenResult>();
    screen.onResult(resultRef::set);
    screen.open();
