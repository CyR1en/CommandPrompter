...
class ItemScreenLayoutTest {
    ...
    void testIsCancelSlot() {
        ...
        for (ItemScreenMode mode : new ItemScreenMode[] {
            ItemScreenMode.INVENTORY, ItemScreenMode.ARMOR, ItemScreenMode.CATALOG
        }) {
            assertTrue(ItemScreenLayout.isCancelSlot(mode, 53), "Slot 53 should be cancel slot for " + mode);
            assertFalse(ItemScreenLayout.isCancelSlot(mode, 8), "Slot 8 should not be cancel slot for " + mode);
            assertFalse(ItemScreenLayout.isCancelSlot(mode, 0), "Slot 0 should not be cancel slot for " + mode);
        }
    }
    ...
    void testIsSelectableInventoryMode() {
        ...
        for (int slot = 0; slot <= 26; slot++) {
            assertTrue(ItemScreenLayout.isSelectableGuiSlot(ItemScreenMode.INVENTORY, slot), "Slot " + slot + " should be selectable in INVENTORY");
        }
        // Hotbar: 27..35
        ...
        for (int slot = 36; slot <= 44; slot++) {
            assertFalse(ItemScreenLayout.isSelectableGuiSlot(ItemScreenMode.INVENTORY, slot), "Slot " + slot + " should NOT be selectable in INVENTORY");
        }
        // Equipment: 45..49
    ...
    void testIsSelectableArmorMode() {
        for (int slot = 0; slot <= 44; slot++) {
            assertFalse(ItemScreenLayout.isSelectableGuiSlot(ItemScreenMode.ARMOR, slot), "Slot " + slot + " should NOT be selectable in ARMOR");
        }
        for (int slot = 45; slot <= 49; slot++) {
            assertTrue(ItemScreenLayout.isSelectableGuiSlot(ItemScreenMode.ARMOR, slot), "Slot " + slot + " should be selectable in ARMOR");
        }
        for (int slot = 50; slot <= 53; slot++) {
            assertFalse(ItemScreenLayout.isSelectableGuiSlot(ItemScreenMode.ARMOR, slot), "Slot " + slot + " should NOT be selectable in ARMOR");
        }
    ...
    void testIsSelectableCatalogMode() {
        for (int slot = 0; slot <= 44; slot++) {
            assertTrue(ItemScreenLayout.isSelectableGuiSlot(ItemScreenMode.CATALOG, slot), "Slot " + slot + " should be selectable in CATALOG");
        }
        for (int slot = 45; slot <= 53; slot++) {
            assertFalse(ItemScreenLayout.isSelectableGuiSlot(ItemScreenMode.CATALOG, slot), "Slot " + slot + " should NOT be selectable in CATALOG");
        }
    }
