...
class ConfirmationRateLimiterTest {
    ...
    void slidingWindowRollsOffExpiredAttempts() {
        ...
        assertTrue(rateLimiter.tryAcquire(player1), "New attempt should succeed after previous window expired");
    }
    ...
    void separateSuppressionChannelsAreIndependent() {
        ...
        assertFalse(rateLimiter.shouldLogRejection(player1), "Second rejection log in window should be suppressed");

        // Rate limit log is on a separate channel and still allowed
        assertTrue(rateLimiter.shouldLogRateLimit(player1), "Rate limit log should still be allowed after prior rejection");
        assertFalse(rateLimiter.shouldLogRateLimit(player1), "Second rate limit log in window should be suppressed");
    ...
    void resetClearsAllSuppressionChannelsForPlayer() {
        ...
        assertTrue(rateLimiter.shouldLogRejection(player1), "Rejection log should be allowed after reset");
        assertTrue(rateLimiter.shouldLogRateLimit(player1), "Rate limit log should be allowed after reset");
    }
    ...
    void purgeExpiredClearsChannelSuppression() {
        ...
        assertTrue(rateLimiter.shouldLogRejection(player1), "Rejection log should be allowed after window expires");
        assertTrue(rateLimiter.shouldLogRateLimit(player1), "Rate limit log should be allowed after window expires");
    }
    ...
    void clearResetsAllSuppressionChannels() {
        ...
        assertTrue(rateLimiter.shouldLogRejection(player1), "Rejection log should be allowed after clear");
        assertTrue(rateLimiter.shouldLogRateLimit(player1), "Rate limit log should be allowed after clear");
    }
