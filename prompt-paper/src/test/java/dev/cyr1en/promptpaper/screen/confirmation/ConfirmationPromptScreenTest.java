...
class ConfirmationPromptScreenTest {
  ...
  void cancelOutcomeDeliversCancelReason() {
    ...
    assertEquals(
        ConfirmationOutcome.cancelled(CancelReason.TIMEOUT), screen.lastOutcome().orElse(null));
  }
  ...
  void metadataAndAccessors() {
    var screen = new ConfirmationPromptScreen(List.of(primaryView, fallbackView), "OK", "NO", true);
