...
class CustomScreenTokenGateRaceTest {
    ...
    @Test
    @DisplayName("Gate Race: Factory invocation vs Teardown - In-flight factory completes before state transition")
    void testFactoryInvocationVsTeardownRaceInFlightCompletes() throws Exception {
        ...
        PromptScreenFactory slowFactory = (player, context) -> {
        ...
        Future<InputScreen> factoryFuture = executor.submit(() ->
        ...
        // While factory is blocked in gate, token should still be seen as ACTIVE by the thread holding the gate
    ...
    @Test
    @DisplayName("Gate Race: Factory invocation vs Teardown - Teardown wins before factory starts, factory call count zero")
    void testFactoryInvocationVsTeardownRaceTeardownWins() throws Exception {
        AtomicInteger factoryCallCount = new AtomicInteger(0);
        PromptScreenFactory factory = (player, context) -> {
            factoryCallCount.incrementAndGet();
    ...
    @Test
    @DisplayName("Gate Race: Delegate open vs Teardown - In-flight open completes before state transition")
    void testDelegateOpenVsTeardownRaceInFlightCompletes() throws Exception {
        ...
        MockInputScreen delegate = new MockInputScreen() {
            @Override
            public void open() {
        ...
        CustomScreenToken token = new CustomScreenToken(1L, "openscreen", "OpenPlugin", (p, c) -> delegate);
    ...
    @Test
    @DisplayName("Gate Race: Delegate close vs Teardown - In-flight close completes before state transition")
    void testDelegateCloseVsTeardownRaceInFlightCompletes() throws Exception {
        ...
        CustomScreenToken token = new CustomScreenToken(1L, "closescreen", "ClosePlugin", (p, c) -> delegate);
    ...
    @Test
    @DisplayName("Gate Race: Delegate isOpen vs Teardown - In-flight isOpen completes before state transition")
    void testDelegateIsOpenVsTeardownRaceInFlightCompletes() throws Exception {
        ...
        CustomScreenToken token = new CustomScreenToken(1L, "isopenscreen", "IsOpenPlugin", (p, c) -> delegate);

        Future<Boolean> isOpenFuture = executor.submit(() ->
                token.callIfActive(delegate::isOpen).orElse(false)
        );
    ...
    @Test
    @DisplayName("Gate Race: Delegate Callback Registration vs Teardown - In-flight callback registration completes before state transition")
    void testDelegateCallbackRegistrationVsTeardownRace() throws Exception {
        ...
        CustomScreenToken token = new CustomScreenToken(1L, "regscreen", "RegPlugin", (p, c) -> delegate);

        Future<Boolean> regFuture = executor.submit(() ->
                token.runIfActive(() -> delegate.onResult(res -> {}))
        );
    ...
    void testCustomScreenAdapterLazyOpenVsTeardownRace() throws Exception {
        ...
        CustomScreenToken token = new CustomScreenToken(1L, "adapterrace", "AdapterPlugin", (p, ctx) -> {
        ...
        CustomScreenAdapter adapter = CustomScreenAdapter.lazy(
                token,
                () -> token.invokeFactory(f -> f.createScreen(null, null)).orElse(null),
                directExecutor,
                snapshot,
        ...
        assertEquals(ProviderState.INACTIVE, token.state());
        // Since teardown ran right after factory creation, open either executed before teardown or was safely gated out (openCalls is 0 or 1, never called while inactive)
