...
class CustomScreenRuntimeTest extends MockBukkitTest {
  ...
  @Test
  @DisplayName(
      "API-04/05: Registered custom tag creates ScreenContext with exact display/flags/sanitize and factory called on player executor")
  void testCustomTagCreatesScreenContextAndFactoryRunsOnPlayerExecutor() {
    ...
    PromptScreenFactory customFactory =
        (p, ctx) -> {
    ...
    assertEquals(
        mockScreen, ((CustomScreenAdapter) screenManager.getActiveScreen(player)).delegate());
  }

  @Test
  @DisplayName(
      "SEC-12/API-06: Factory returning null fails closed with one CancelReason.ERROR and no command dispatch")
  ...
  @Test
  @DisplayName(
      "SEC-12/API-06: Factory throwing exception fails closed with one CancelReason.ERROR and no primary dispatch")
  void testFactoryThrowingExceptionCancelsWithError() {
    PromptScreenFactory throwingFactory =
        (p, ctx) -> {
  ...
  @Test
  @DisplayName(
      "SEC-12/API-06: Delegate open() throwing exception fails closed with CancelReason.ERROR")
  ...
  void testValidationRetryInvalidatesOldAttemptToken() {
    ...
    assertEquals(
        firstScreen, ((CustomScreenAdapter) screenManager.getActiveScreen(player)).delegate());
    ...
    assertEquals(
        secondScreen,
        ((CustomScreenAdapter) screenManager.getActiveScreen(player)).delegate(),
        "New attempt must have replacement screen");
    ...
    assertEquals(
        secondScreen, ((CustomScreenAdapter) screenManager.getActiveScreen(player)).delegate());
    assertTrue(engine.hasActiveSession(player));
  ...
  void testNormalCompletionAndCancelUnlinksReverseIndex() {
    ...
    assertEquals(
        ...
        "Reverse index must be empty after cancel");
  }
