...
class PromptConfigTest extends MockBukkitTest {
  ...
  private PromptConfig makeDefaultCfg() {
    return new PromptConfig(
        ...
        true,
        "&7[&c&l✘&7]",
        "&7Click here to cancel command completion",
        "LOWEST",
        "bottom",
        "OAK_SIGN",
        "is",
        "^\\d+",
        "&cPlease enter a valid integer!",
        "ss",
        "[A-Za-z ]+",
        "&cInput must only consist letters of the alphabet!",
        "Prompt",
        "<green>Confirm</green>",
        "Confirm this action",
        "<red>Cancel</red>",
  ...
  void dialogConfigBuildsNestedRecordsFromFields() {
    ...
    assertTrue(
        dialog.choice().defaultOptions().isEmpty(),
  ...
  void hasValidatorReturnsTrueForConfiguredAlias() {
    ...
    when(rawConfig.getKeys("Input-Validation.Integer-Sample"))
        .thenReturn(Set.of("Alias", "Regex", "Err-Message"));
  ...
  void getInputValidatorThrowsOnUnknownAlias() {
    ...
    assertThrows(
        IllegalArgumentException.class, () -> cfg.getInputValidator("unknown", null, null));
    assertInstanceOf(
        dev.cyr1en.promptpaper.validation.NoopValidator.class,
        cfg.getInputValidator(null, null, null));
    assertInstanceOf(
        dev.cyr1en.promptpaper.validation.NoopValidator.class,
        cfg.getInputValidator("", null, null));
  }
