...
/**
 * A rectangular grid of {@link GuiItem}s managed through a collection of {@link Pane} objects.
 *
 * <p>Panes are rendered in priority order (lowest first) into a merged {@link GuiItemContainer}.
 * Direct items placed via {@link #addItem} are stored in a background {@link StaticPane} (priority
 * 0).
 */
public final class GuiComponent {
  ...
  /**
   * Adds a pane at the given offset within the component. The pane's priority determines render
   * order.
   */
  ...
  public List<Pane> getPanes() {
    return panes.stream().map(PositionedPane::pane).toList();
  }
  ...
  public GuiItemContainer display() {
    ...
    mergeContainer(container, itemsStaticPane.display(), 0, 0);
    panes.stream()
        .sorted()
        ...
        .forEach(
            pp -> {
              ...
              mergeContainer(container, paneOutput, pp.offset().x(), pp.offset().y());
            });
    return container;
  }
  ...
  public boolean click(@NotNull Gui gui, @NotNull InventoryClickEvent event, int rawSlot) {
    ...
    List<PositionedPane> renderedPanes =
        panes.stream().filter(pp -> pp.pane().isVisible()).sorted().toList();
    for (int i = renderedPanes.size() - 1; i >= 0; i--) {
      ...
      if (localX < 0
          || localX >= pp.pane().getLength()
          || localY < 0
          || localY >= pp.pane().getHeight()) {
        continue;
      }
  ...
  /**
   * Writes rendered items into a player inventory (bottom part of view). Player inventory slots are
   * laid out in reversed row order.
   */
  ...
  private static void mergeContainer(
      @NotNull GuiItemContainer target,
