...
/**
 * An anvil GUI with three input slots (first item, second item, result) and a rename text field.
 *
 * <p>The actual anvil inventory is created via a version-specific {@link AnvilInventory}
 * implementation provided at construction time.
 */
public final class AnvilGui extends NamedGui implements InventoryBased {
  ...
  @Nullable private Consumer<InventoryClickEvent> onResultClick;
  ...
  public void show(@NotNull HumanEntity humanEntity) {
    ...
    if (!nameInputSubscribed) {
      anvilInventory.subscribeToNameInputChanges(
          text -> {
  ...
  /**
   * Sets a direct callback for result-slot clicks, bypassing UUID matching (necessary because NMS
   * overwrites the result ItemStack, stripping the UUID tag that the pane system relies on).
   */
