 ...
 *
 * <p>Provides the core lifecycle ({@link #show}, {@link #update}, {@link #click}), event callbacks,
 * and a static registry mapping {@link Inventory} instances to their owning {@link Gui}. The
 * registry is explicit because Bukkit refuses to set {@link InventoryHolder} on certain inventory
 * types (e.g., anvils). Entries are removed when the last viewer closes the inventory.
 */
public abstract class Gui {
  ...
  /**
   * Opens this GUI for the given player. Caches the player's inventory first, then opens the
   * inventory. Subclasses must call {@code super.show(humanEntity)} after preparing the inventory.
   */
  ...
  /**
   * Returns whether this GUI has pending changes requiring re-render. Always returns {@code true};
  ...
  /**
   * Removes an inventory association if it still belongs to the supplied GUI. The conditional form
   * prevents an old GUI from removing a newer mapping.
   */
  ...
  public static Gui getGui(@NotNull Inventory inventory) {
    ...
    if (holder instanceof Gui holderGui) {
      return holderGui;
    }
    return null;
  }
  ...
  /** Removes this GUI from the explicit registry only after its final viewer has gone away. */
