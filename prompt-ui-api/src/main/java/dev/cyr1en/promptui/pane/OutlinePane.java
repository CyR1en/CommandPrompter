...
/**
 * A pane that lays out items in reading order (horizontally or vertically) respecting a {@link
 * Mask} that controls which cells are enabled.
 *
 * <p>Items are placed sequentially into enabled mask cells. If there are more items than enabled
 * cells, extra items are not displayed. Pagination can be handled by a parent {@link
 * PaginatedPane}.
 */
public final class OutlinePane extends Pane {
  ...
  public boolean click(
      ...
      @NotNull Slot slot) {
    GuiItem item = display().getItem(slot);
    if (item != null) {
