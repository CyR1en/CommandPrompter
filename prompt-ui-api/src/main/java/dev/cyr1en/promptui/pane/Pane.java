 ...
 *
 * <p>A pane occupies a rectangular region of a {@link GuiComponent} and manages its own set of
 * {@link GuiItem} placements. Panes are rendered by priority (lowest first).
 */
  ...
  protected Pane(int length, int height) {
    if (length <= 0 || height <= 0) {
      throw new IllegalArgumentException(
          "Pane dimensions must be positive, got " + length + "x" + height);
    }
    this.length = length;
    this.height = height;
