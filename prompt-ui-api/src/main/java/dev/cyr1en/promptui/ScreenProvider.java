 ...
 *
 * <p>Discovered via {@link java.util.ServiceLoader}. The {@code prompt-ui-<ver>} modules each
 * register a concrete implementation under {@code
 * META-INF/services/dev.cyr1en.promptui.ScreenProvider}. At runtime, the consumer in {@code
 * prompt-paper} calls {@code ServiceLoader.load(ScreenProvider.class, ...)} to obtain the matching
 * implementation for the current Minecraft version.
 *
 * <p>Implementations are not required to be thread-safe. The Bukkit/Paper API bound here means that
 * the SPI is currently usable only on Bukkit-based servers.
 */
public interface ScreenProvider {
  ...
  /**
   * Returns the target Minecraft version this provider was built for (e.g., "26.1"). Used to warn
   * server admins if they are running a newer server version without a dedicated NMS module.
   *
