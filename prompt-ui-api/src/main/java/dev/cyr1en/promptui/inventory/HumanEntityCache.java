 ...
 *
 * <p>Before opening a GUI, the player's inventory (slots 0-35) is saved and cleared so the
 * framework can place items in the bottom inventory without the player's own items interfering. On
 * close, the original items are restored.
 */
public final class HumanEntityCache {

  /**
   * The cache is accessed by inventory events and by player-affine tasks. A concurrent map keeps
   * the lifecycle safe when those paths happen on different region threads; the individual
   * snapshots are synchronized when their slots are modified or restored.
   */
  ...
  /**
   * Adds an item to the cached inventory if the player is currently cached. Used to prevent the
   * player picking up items dropped during GUI interaction.
   *
  ...
  /**
   * Restores every cached inventory and clears the cache. This is used by plugin-wide GUI shutdown
   * so a close event is not required for cleanup.
   */
